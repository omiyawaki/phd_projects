...
for m in range(len(models)):
    # RRTMG climatology
    ref = {}
    ref['ra']= np.empty([len(models), nref])
    ref['racs']= np.empty([len(models), nref])
    ref['diag']= np.empty([len(models), nref])
    [ref['ra'][m,:], ref['racs'][m,:], ref['diag'][m,:]] = pickle.load(open('%s/rad.hist.%s.pickle' % (datadir, model), 'rb'))
    print(ref['ra'].shape)
