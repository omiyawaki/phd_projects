...
for model in models:
    ...
    fixqt = {}
    [fixqt['ra'], fixqt['racs'], fixqt['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixT.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixqt_indiv = {}
        fixqt_indiv['racs'] = np.empty([len(allmodels), len(fixqt['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixT.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    fixqco2 = {}
    [fixqco2['ra'], fixqco2['racs'], fixqco2['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixco2.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixqco2_indiv = {}
        fixqco2_indiv['racs'] = np.empty([len(allmodels), len(fixqco2['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixco2.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    fixqco2_planck = {}
    [fixqco2_planck['ra'], fixqco2_planck['racs'], fixqco2_planck['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixco2_planck.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixqco2_planck_indiv = {}
        fixqco2_planck_indiv['racs'] = np.empty([len(allmodels), len(fixqco2_planck['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixco2_planck.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    fixqco2_lapse = {}
    [fixqco2_lapse['ra'], fixqco2_lapse['racs'], fixqco2_lapse['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixco2_lapse.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixqco2_lapse_indiv = {}
        fixqco2_lapse_indiv['racs'] = np.empty([len(allmodels), len(fixqco2_lapse['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixq_fixco2_lapse.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    fixtco2 = {}
    [fixtco2['ra'], fixtco2['racs'], fixtco2['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixT_fixco2.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixtco2_indiv = {}
        fixtco2_indiv['racs'] = np.empty([len(allmodels), len(fixtco2['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixT_fixco2.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    fixrhco2 = {}
    [fixrhco2['ra'], fixrhco2['racs'], fixrhco2['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixrh_fixco2.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixrhco2_indiv = {}
        fixrhco2_indiv['racs'] = np.empty([len(allmodels), len(fixrhco2['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixrh_fixco2.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    fixtco2devrh = {}
    [fixtco2devrh['ra'], fixtco2devrh['racs'], fixtco2devrh['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixT_fixco2_devrh.%s.cmip6.pickle' % (datadir, model), 'rb'))
    if model == 'mmm':
        fixtco2devrh_indiv = {}
        fixtco2devrh_indiv['racs'] = np.empty([len(allmodels), len(fixtco2devrh['racs'])])
        for im in range(len(allmodels)):
            [ra, racs, diag] = pickle.load(open('%s/rad.hist+ssp585_fixT_fixco2_devrh.%s.cmip6.pickle' % (datadir, allmodels[im]), 'rb'))
    ...
    # fixco2 = {}
    # [fixco2['ra'], fixco2['racs'], fixco2['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixco2.%s.cmip6.pickle' % (datadir, model), 'rb'))

    # # RRTMG fixed specific humidity RCP8.5 response
    # fixq = {}
    # [fixq['ra'], fixq['racs'], fixq['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixq.%s.cmip6.pickle' % (datadir, model), 'rb'))

    # # RRTMG fixed relative humidity RCP8.5 response
    # fixrh = {}
    # [fixrh['ra'], fixrh['racs'], fixrh['diag']] = pickle.load(open('%s/rad.hist+ssp585_fixrh.%s.cmip6.pickle' % (datadir, model), 'rb'))
    ...
    plotname='%s/dracs_fixq_fixT.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_fixq_fixco2.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ax.plot(time, full['racs']-ref['racs'], color='black', label='RRTMG')
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_dt_dc_planck_lapse.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    if model == 'mmm':
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_fixT_fixco2.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ax.plot(time, full['racs']-ref['racs'], color='black', label='RRTMG')
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_fixrh_fixco2.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ax.plot(time, full['racs']-ref['racs'], color='black', label='RRTMG')
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_decomp_noleg.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ...
    lt,=ax.plot(time, fixqco2['racs']-ref['racs'], '-', color='tab:orange', label='$\Delta T$')
    lq,=ax.plot(time, fixtco2['racs']-ref['racs'], '-', color='tab:blue', label='$\Delta q$')
    # ax.set_xlim([time[0], time[-1]])
    ax.set_xlim([1987, 2200])
    ax.xaxis.set_minor_locator(AutoMinorLocator())
    ax.yaxis.set_minor_locator(AutoMinorLocator())
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_decomp.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    if model == 'mmm':
        ax.fill_between(time, full['racs']-ref['racs']-full_mmm['std']/sqn, full['racs']-ref['racs']+full_mmm['std']/sqn, alpha=0.2, color='k', edgecolor=None)
    ...
    fig.set_size_inches(4,3.3)
    plt.tight_layout()
    leg=plt.legend(handles=[lfull,lco2,lt,lq,lres], loc='lower left', prop={'size':10})
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_decomp_leg.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    lres,=ax.plot(time, full['racs']-fixqt['racs']-fixqco2['racs']-fixtco2['racs']+2*ref['racs'], '-.', color='k', label='Residual')
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_decomp_rh_noleg.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_decomp_rh.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    if model == 'mmm':
        ax.fill_between(time, full['racs']-ref['racs']-full_mmm['std']/sqn, full['racs']-ref['racs']+full_mmm['std']/sqn, alpha=0.2, color='k', edgecolor=None)
    ...
    ##################################################
    # compare change in radiative cooling
    plotname='%s/dracs_decomp_rh_leg.cmip6.pdf' % (plotdir)
    fig, ax = plt.subplots()
    ax.axhline(0,color='k', linewidth=0.5)
    ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    lres,=ax.plot(time, full['racs']-fixqt['racs']-fixrhco2['racs']-fixtco2devrh['racs']+2*ref['racs'], '-.', color='k', label='Residual')
    ...
    #plotname='%s/co2_racs_fixq_fixT.cmip6.pdf' % (plotdir)
    #fig, ax = plt.subplots()
    #ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ...
    ###################################################
    ## compare absolute change in radiative cooling
    #plotname='%s/racs_fixq.cmip6.pdf' % (plotdir)
    #fig, ax = plt.subplots()
    #ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    ...
    ###################################################
    ## compare absolute change in radiative cooling
    #plotname='%s/racs_fixrh.cmip6.pdf' % (plotdir)
    #fig, ax = plt.subplots()
    #ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    #ax.plot(time, gcm_forc['racs'], color='tab:red', label=modelstr)
    #ax.plot(time, full['racs'], color='black', label='RRTMG')
    ...
    ###################################################
    ## compare absolute change in radiative cooling
    #plotname='%s/racs_fixrh_fixco2.cmip6.pdf' % (plotdir)
    #fig, ax = plt.subplots()
    #ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    #ax.plot(time, gcm_forc['racs'], color='tab:red', label=modelstr)
    #ax.plot(time, full['racs'], color='black', label='RRTMG')
    ...
    ###################################################
    ## compare absolute change in radiative cooling
    #plotname='%s/racs_fixco2.cmip6.pdf' % (plotdir)
    #fig, ax = plt.subplots()
    #ax.tick_params(which='both', bottom=True, top=True, left=True, right=True)
    #ax.plot(time, gcm_forc['racs'], color='tab:red', label=modelstr)
    #ax.plot(time, full['racs'], color='black', label='RRTMG')
