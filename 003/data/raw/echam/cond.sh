...
for sim in ${sims[@]}; do
    ...
    if ls ${indir}/cond_${common} 1> /dev/null 2>&1; then # check if data is already there
        echo "conductive heat flux was already created. Skipping..."
    else
        ...
        python ${cwd}/make_cond.py ${full_tsurf} ${full_tsi} ${full_siced} ${full_sni} ${full_cond}
    fi
