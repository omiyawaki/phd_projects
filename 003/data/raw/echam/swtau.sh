...
for model in ${models[@]}; do
    ...
    if ls $cwd/${model}/swtau_${common} 1> /dev/null 2>&1; then # check if data is already there
        echo "mmc and se was already created. Skipping..."
    else

        full_sradsd=${cwd}/${model}/sradsd_${common}.nc
        full_srad0d=${cwd}/${model}/srad0d_${common}.nc
        full_swtau=${cwd}/${model}/swtau_${common}.nc

        python ${cwd}/make_swtau.py ${full_sradsd} ${full_srad0d} ${full_swtau}
    fi
