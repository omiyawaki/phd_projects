...
# read data
ps = file_ps.variables['aps'][:] # (yr x lat x lon)
va = file_atm.variables['v'][:] # (mon x lev x lat x lon)
mse = file_mse.variables['mse'][:] # (mon x lev x lat x lon)
mses = file_mses.variables['mses'][:] # (yr x lat x lon)
vas = file_vas.variables['vas'][:] # (yr x lat x lon)
lat = file_atm.variables['lat'][:] # (lat)
plev = file_atm.variables['lev'][:] # (lev)

ps = np.tile(ps, (mse.shape[0],1,1))
vas = np.tile(vas, (mse.shape[0],1,1))
mses = np.tile(mses, (mse.shape[0],1,1))
