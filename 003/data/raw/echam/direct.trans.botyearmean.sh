    ...
    for yr in $(seq $yr_begin $yr_end); do
        for mon in $(seq -f "%02g" ${mn_begin} ${mn_end}); do
            ...
            if ls $cwd/${sim}/vmte_${common} 1> /dev/null 2>&1; then # check if data is already there
                ...
                full_mses=${cwd}/${sim}/mses_${common_ml}.yearmean.nc
                ...
                full_vas=${cwd}/${sim}/vas_${common_ml}.yearmean.nc
                # full_ps=/project2/tas1/ockham/data11/tas/echam-aiv_rcc_6.1.00p1/${sim}/ATM_${common_ml}.nc
                full_ps=${cwd}/${sim}/BOT_${sim}_${yr}.yearmean.nc
                ...
                python ${cwd}/make_direct.trans.${lev}.botyearmean.py ${full_atm} ${full_mses} ${full_mse} ${full_vmmmc} ${full_vmse} ${full_vmte} ${full_vas} ${full_ps} ${full_va}
