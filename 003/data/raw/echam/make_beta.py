...
ta = np.squeeze(file_ta.variables['t'][:]) # (mon x lev x lat x lon)
print(ps.shape)
print(ta.shape)
...
b = np.empty( [1, ta.shape[1], ta.shape[2], ta.shape[3]] )
...
file_beta.setncatts(file_ta.__dict__)
...
for name, variable in file_ta.variables.items():
    if any(name in s for s in ['t']): # don't copy va data
        continue
    ...
    else: # copy all other axis data from ta
        x = file_beta.createVariable(name, variable.datatype, variable.dimensions)
        file_beta[name].setncatts(file_ta[name].__dict__)
        file_beta[name][:] = file_ta[name][:]
    
try:
    beta = file_beta.createVariable('beta', 'f4', ("time","plev","lat","lon"))
except:
    beta = file_beta.createVariable('beta', 'f4', ("time","lev","lat","lon"))
beta.units = "unitless"
