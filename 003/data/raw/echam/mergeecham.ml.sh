...
declare -a clims=("rp000201") # climate name
yr0=0040
yr1=0083
...
for dirs in ${clims[@]}; do # loop through models
    ...
    for vars in ${vars_gcm[@]}; do
        ...
        ##########################################################
            ...
            pattern=$(ls ${dim}_${dirs}_{0040..0083}.nc)
            ...
            if [[ ! -f "$cwd/$dirs/${dim}_${dirs}_${yr0}_${yr1}.ml.nc" ]]; then # check if merged file already exists
                cdo -O mergetime ${files[@]} $cwd/$dirs/${dim}_${dirs}_${yr0}_${yr1}.merge.nc # combine multiple files into one
                cdo -O monmean $cwd/$dirs/${dim}_${dirs}_${yr0}_${yr1}.merge.nc $cwd/$dirs/${dim}_${dirs}_${yr0}_${yr1}.nc # monthly means
                rm $cwd/$dirs/${dim}_${dirs}_${yr0}_${yr1}.merge.nc 
            fi

            cdo -O selvar,$vars $cwd/$dirs/${dim}_${dirs}_${yr0}_${yr1}.nc $cwd/$dirs/${vars}ml_${dirs}_${yr0}_${yr1}.nc 
        # fi
        cd $rwd/$dirs # go in the model directory
    done
