...
hus = file_hus.variables['q'][:] # (mon x lev x lat x lon)
try:
    plev = file_hus.variables['plev'][:]
except:
    plev = file_hus.variables['lev'][:]
...
try:
    husz = file_husz.createVariable('q', 'f4', ("time","plev","lat"))
except:
    husz = file_husz.createVariable('q', 'f4', ("time","lev","lat"))
husz.units = "kg/kg"
