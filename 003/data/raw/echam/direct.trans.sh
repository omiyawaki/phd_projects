    ...
    for yr in $(seq $yr_begin $yr_end); do
        for mon in $(seq -f "%02g" ${mn_begin} ${mn_end}); do
            ...
            if ls $cwd/${sim}/vmte_${common} 1> /dev/null 2>&1; then # check if data is already there
                ...
                full_bot=${cwd}/${sim}/BOT_dm_${sim}_${yr}${mon}.nc
                ...
                # srun --partition=tas1 --time=6:00:00 --exclusive --pty python ${cwd}/make_direct.trans.${lev}.py ${full_atm} ${full_bot} ${full_mse} ${full_vmmmc} ${full_vmse} ${full_vmte}
                python ${cwd}/make_direct.trans.${lev}.py ${full_atm} ${full_bot} ${full_mse} ${full_vmmmc} ${full_vmse} ${full_vmte}
            fi
