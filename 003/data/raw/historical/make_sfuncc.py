...
# va_z = np.divide( np.nansum(beta*va, axis=3, keepdims=True), deriv_z, out=np.zeros([va.shape[0],va.shape[1],va.shape[2]]), where=deriv_z!=0)
...
# compute deviation of va from vertical mean
va_dev = va_z - va_zv
deriv_z = np.squeeze(deriv_z, axis=3)
beta_z = np.squeeze(beta_z, axis=3)
va_dev = np.divide( np.nansum(beta*va_dev, axis=3), deriv_z, out=np.zeros([va.shape[0],va.shape[1],va.shape[2]]), where=deriv_z!=0)

# compute streamfunction
rlat = np.radians(lat3d)
clat = np.cos(rlat)
if plev[1]-plev[0]>0: # if pressure increases with index
    psi = 2*np.pi*a*clat[None,None,:]/g * np.cumsum( beta_z * va_dev * dplev[None,:,None], axis=1)
else:
    psi = -2*np.pi*a*clat[None,None,:]/g * np.cumsum( beta_z[:,::-1,:] * va_dev[:,::-1,:] * dplev[None,::-1,None], axis=1)
    psi = psi[:,::-1,:]
...
sfunc = file_psi.createVariable('psic', 'f4', ("time","plev","lat"))
sfunc.units = "kg/s"
sfunc.long_name = "mean meridional mass streamfunction (mass-corrected)"
sfunc[:,:,:] = psi
