...
aht_file = Dataset('./aht_Amon_ACCESS1-0_piControl_r1i1p1_075001-079912.timmean.nc', 'r')
aht = aht_file.variables['aht'][:][0,...]
# aht = np.squeeze(np.mean(aht_file.variables['aht'][:],axis=0))

vmmmc_file = Dataset('./vmmmc_Amon_ACCESS1-0_piControl_r1i1p1_075001-079912.timmean.nc', 'r')
vmmmc = vmmmc_file.variables['vmmmc'][:][0,...]
# vmmmc = np.squeeze(np.mean(vmmmc_file.variables['vmmmc'][:],axis=0))

vmse_file = Dataset('./vmse_Amon_ACCESS1-0_piControl_r1i1p1_075001-079912.timmean.nc', 'r')
vmse = vmse_file.variables['vmse'][:][0,...]
# vmse = np.squeeze(np.mean(vmse_file.variables['vmse'][:],axis=0))

vmte_file = Dataset('./vmte_Amon_ACCESS1-0_piControl_r1i1p1_075001-079912.timmean.nc', 'r')
...
ax.set_title('ACCESS1-0 piControl')
