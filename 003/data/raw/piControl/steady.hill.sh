...
for model in ${models[@]}; do
    ...
    if ls $cwd/${model}/vmmmc_${common} 1> /dev/null 2>&1; then # check if data is already there
        ...
        # srun --partition=tas1 --time=6:00:00 --exclusive --pty python ${cwd}/make_steady.hill.py ${full_ps} ${full_va} ${full_vas} ${full_mse} ${full_mses} ${full_vmmmc} ${full_vmse}
        python ${cwd}/make_steady.hill.py ${full_ps} ${full_va} ${full_vas} ${full_mse} ${full_mses} ${full_vmmmc} ${full_vmse}
    fi
