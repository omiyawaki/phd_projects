...
# Uniform dse v offset test (acts as a sanity check because transport should be insensitive to constant V or dse offsets if analysis is implemented correctly)
...
file_dse = Dataset(path_dse, 'r')
file_cbeta = Dataset(path_cbeta, 'r')

# Read data
va = file_va.variables['va'][:] # (mon x lev x lat x lon)
dse = file_dse.variables['dse'][:] # (mon x lev x lat x lon)
beta = file_cbeta.variables['beta'][:] # (1 x lev x lat x lon) climatological beta

################################################################################
# Uniform dse v offset test 
################################################################################
if do_uniform_offset:
    print('Adding uniform %g J/kg to dse field and %g m/s to v field...' % (dse_offset, va_offset))
    dse = dse + dse_offset
    va = va + va_offset
...
if method_subsurface == 1:
    ...
    mask = ~np.isnan(dse.filled(fill_value=np.nan))
    beta = beta * mask
    ...
    dse = dse.filled(fill_value=np.nan)
    if np.any(np.isnan(dse)):
        dse = xr.open_dataset(path_dse).dse # (mon x plev x lat x lon)
        dse = dse.reindex(plev=dse['plev'][::-1])
        dse = dse.interpolate_na(dim="plev", method='linear', fill_value="extrapolate")
        dse = dse.reindex(plev=dse['plev'][::-1])
        dse = dse.values
...
dse_z = np.divide( np.nansum(beta*dse, axis=3, keepdims=True), deriv_z, out=np.zeros_like(dse), where=deriv_z!=0)
beta_z = np.mean(beta, axis=3, keepdims=True)
deriv_z = np.squeeze(deriv_z)

################################################################################
# Compute stationary eddy (SE) dse transport
################################################################################
vm_se_lon = (va - va_z) * (dse - dse_z)
vm_se = np.divide( np.nansum(beta*vm_se_lon, axis=3), deriv_z, out=np.zeros(dse.shape[:-1]), where=deriv_z!=0)

################################################################################
# Compute mean meridional circulation (MMC) dse transport
################################################################################
...
# compute deviation of vertical mean zonal mean v and dse
...
dse_zv = np.divide( np.nansum(beta*dse*dplev_ext, axis=(1,3), keepdims=True), denom_zv, out=np.zeros_like(dse), where=denom_zv!=0)

# compute mean meridional circulation (MMC) transport following Marshall et al. (2014)
vm_mmc_lon = np.squeeze( (va_z - va_zv) * (dse_z - dse_zv) )
vm_mmc = np.divide( np.nansum(beta*vm_mmc_lon, axis=3), deriv_z, out=np.zeros(dse.shape[:-1]), where=deriv_z!=0)
...
# copy attributes from dse file
file_mmc.setncatts(file_dse.__dict__)
file_se.setncatts(file_dse.__dict__)
...
mmc = file_mmc.createVariable('vvsmmc', 'f4', ("time","plev","lat"))
mmc.units = "W m**-1"
mmc.long_name = "meridional dry static energy flux transport due to mean meridional circulation"
mmc[:] = vm_mmc

se = file_se.createVariable('vvsse', 'f4', ("time","plev","lat"))
se.units = "W m**-1"
se.long_name = "meridional dry static energy flux transport due to stationary eddies"
se[:] = vm_se
