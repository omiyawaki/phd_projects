...
dse_z = np.nanmean(dse, axis=3, keepdims=True)
beta_z = np.nanmean(beta, axis=3, keepdims=True)

# compute stationary eddy (SE) dse transport
vs_se = np.nanmean((va - va_z) * (dse - dse_z), axis=3)

# vertically and zonally integrate SE dse transport
...
vs_se_z_vint = a*clat_ext*dlon/g * np.nansum(beta*vs_se[...,None]*dplev_ext, axis=(1,3))

# compute deviation of vertical mean zonal mean v and dse (used to compute MMC transport)
denom_zv = np.nansum(beta*dplev_ext, axis=1, keepdims=True)
va_zv = np.nansum(beta*va_z*dplev_ext, axis=1, keepdims=True) / denom_zv
dse_zv = np.nansum(beta*dse_z*dplev_ext, axis=1, keepdims=True) / denom_zv

# compute mean meridional circulation (MMC) transport following Marshall et al. (2014)
vs_mmc = (va_z - va_zv) * (dse_z - dse_zv)

# vertically and zonally integrate MMC dse transport
vs_mmc_z_vint = a*clat_ext*dlon/g * np.nansum(beta*vs_mmc*dplev_ext, axis=(1,3))
...
if plev[1]-plev[0]<0: # if pressure increases with index
    vs_se_z_vint = -vs_se_z_vint
    vs_mmc_z_vint = -vs_mmc_z_vint
...
file_mmc.setncatts(file_va.__dict__)
file_se.setncatts(file_va.__dict__)
...
mmc = file_mmc.createVariable('vsmmc', 'f4', ("time","lat"))
mmc.units = "W"
mmc.long_name = "vertically integrated dry static energy flux energy flux transport due to mean meridional circulation"
mmc[:,:] = vs_mmc_z_vint

se = file_se.createVariable('vsse', 'f4', ("time","lat"))
se.units = "W"
se.long_name = "vertically integrated dry static energy flux transport due to stationary eddies"
se[:,:] = vs_se_z_vint
