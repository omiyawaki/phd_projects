...
################################################################################
vm_se_lon = (va - va_z) * (mse - mse_z)
vm_se = np.divide( np.nansum(beta*vm_se_lon, axis=3), deriv_z, out=np.zeros(mse.shape[:-1]), where=deriv_z!=0)
...
vm_mmc_lon = np.squeeze( (va_z - va_zv) * (mse_z - mse_zv) )
vm_mmc = np.divide( np.nansum(beta*vm_mmc_lon, axis=3), deriv_z, out=np.zeros(mse.shape[:-1]), where=deriv_z!=0)
...
mmc = file_mmc.createVariable('vvmmmc', 'f4', ("time","plev","lat"))
mmc.units = "W m**-1"
mmc.long_name = "meridional moist static energy flux transport due to mean meridional circulation"
mmc[:] = vm_mmc

se = file_se.createVariable('vvmse', 'f4', ("time","plev","lat"))
se.units = "W m**-1"
se.long_name = "meridional moist static energy flux transport due to stationary eddies"
se[:] = vm_se
