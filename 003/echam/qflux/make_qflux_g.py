...
path_swtauni = sys.argv[4]
path_albedoi = sys.argv[5]
path_srad0d = sys.argv[6]
path_qflx = sys.argv[7]
path_qsw = sys.argv[8]

# open files
file_temp = Dataset(path_temp, 'r')
file_fsfc = Dataset(path_fsfc, 'r')
file_swtau = Dataset(path_swtau, 'r')
file_swtauni = Dataset(path_swtauni, 'r')
...
swtau = file_swtau.variables['swtau'][:] # target net surface energy flux, W m**-2 (mon x lat x lon)
swtauni = file_swtauni.variables['swtau'][:] # target net surface energy flux, W m**-2 (mon x lat x lon)
albedoi = file_albedoi.variables['albedo'][:] # target net surface energy flux, W m**-2 (mon x lat x lon)
srad0d = file_srad0d.variables['srad0d'][:] # target net surface energy flux, W m**-2 (mon x lat x lon)

ts=ts.filled(fill_value=np.nan)
fsfc=fsfc.filled(fill_value=np.nan)
swtau=swtau.filled(fill_value=np.nan)
swtauni=swtauni.filled(fill_value=np.nan)
albedoi=albedoi.filled(fill_value=np.nan)
srad0d=srad0d.filled(fill_value=np.nan)
...
qs=((1-albedoo)*np.exp(-swtauni)-(1-albedoi)*np.exp(-swtau))*srad0d
qs[np.isnan(qs)]=0
qflux = qc + qs
