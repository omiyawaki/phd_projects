...
file_pic = Dataset(path_pic, 'r')

# read data
ref = file_ref.variables['aflux'][:] # (mon x lat x lon)
pic = np.flip( np.squeeze(file_pic.variables['q_flux_c'][:]) ) # (mon x lat x lon) qflux

pic = np.transpose( np.tile(pic, [ref.shape[0],ref.shape[2],1]), [0,2,1] )
