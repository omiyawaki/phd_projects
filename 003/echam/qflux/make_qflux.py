...
path_swni = sys.argv[3]
path_swnq = sys.argv[4]
path_qflx = sys.argv[5]

# open files
file_temp = Dataset(path_temp, 'r')
file_fsfc = Dataset(path_fsfc, 'r')
file_swni = Dataset(path_swni, 'r')
file_swnq = Dataset(path_swnq, 'r')
...
swni = file_swni.variables['srads'][:] # ice net surface SW flux, W m**-2 (mon x lat x lon)
swnq = file_swnq.variables['srads'][:] # qflux net surface SW flux, W m**-2 (mon x lat x lon)
...
# compute qflux
qflux = fsfc-rhosea*h*csea*ttend + swnq-swni
