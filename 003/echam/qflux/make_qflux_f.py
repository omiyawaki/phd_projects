...
file_swrefu = Dataset(path_swrefu, 'r')
...
swrefu = file_swrefu.variables['swrefu'][:] # target net surface energy flux, W m**-2 (mon x lat x lon)
...
swrefu=swrefu.filled(fill_value=np.nan)
...
qs=sradsd/(1-albedoo*swrefu)*(albedoi-albedoo)*(1-swrefu)
qs[np.isnan(qs)]=0
qflux = qc + qs
