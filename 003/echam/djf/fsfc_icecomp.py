...
for varname in varnames:
    ...
    ice[varname] = np.nanmean(np.roll(ice[varname],1,axis=0)[0:3,:],axis=0)
...
for varname in varnames:
    ...
    noice[varname] = np.nanmean(np.roll(noice[varname],1,axis=0)[0:3,:],axis=0)
...
# ice['ap'] = np.nanmean(icek['ap'], axis=(0,2))
# noice['ap'] = np.nanmean(noicek['ap'], axis=(0,2))

# # compute pswabs using ice sw down
# ice['pswabs'] = (1-icek['ap'])*icek['srad0d']
# noice['pswabs'] = (1-noicek['ap'])*icek['srad0d']
# ice['pswabs'] = np.nanmean(ice['pswabs'], axis=(0,2))
# noice['pswabs'] = np.nanmean(noice['pswabs'], axis=(0,2))
...
# ice['sswabs'] = np.nanmean(ice['sswabs'], axis=(0,2))
# noice['sswabs'] = np.nanmean(noice['sswabs'], axis=(0,2))
...
for varname in varnames:
    ...
    qflux[varname] = np.nanmean(np.roll(qflux[varname],1,axis=0)[0:3,:],axis=0)
...
# take DJF mean
qflux[varname] = dsvar.data
qflux[varname] = np.nanmean(np.roll(qflux[varname],1)[0:3],axis=0)
...
ax.plot(lat, qflux['tsurf'], '-', color='tab:orange', label=q_label)
ax.set_xticks(np.arange(-90,91,30))
ax.set_xlim([-90,90])
ax.set_xlabel('Latitude (deg)')
ax.xaxis.set_minor_locator(MultipleLocator(10))
...
ax.plot(lat, np.nanmean(noicek['srads']-icek['srads'], axis=(0,2)), '-k', label='ni - i')
ax.set_xlim([-90,90])
ax.set_xticks(np.arange(-90,91,30))
ax.set_xlabel('Latitude (deg)')
ax.xaxis.set_minor_locator(MultipleLocator(10))
...
ax.axhline(0, color='k', linewidth=0.5)
# ax.plot(lat, noice['srads']-ice['srads'], '-k', label='ni - i')
ax.plot(lat, np.nanmean(qfluxk['srads']-icek['srads'], axis=(0,2)), '-k', label='q - i')
ax.set_xlim([-90,90])
ax.set_xticks(np.arange(-90,91,30))
ax.set_xlabel('Latitude (deg)')
ax.xaxis.set_minor_locator(MultipleLocator(10))
...
ax.plot(lat, np.nanmean(noicek['srad0']-icek['srad0'], axis=(0,2)), '-k', label='ni - i')
ax.set_xlim([-90,90])
ax.set_xticks(np.arange(-90,91,30))
ax.set_xlabel('Latitude (deg)')
ax.xaxis.set_minor_locator(MultipleLocator(10))
