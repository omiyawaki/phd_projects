...
def circ_lat_lev(sim, **kwargs):
    ...
    if isinstance(model, str) or model == None:
        ...
        [dyn, _, _, _, _] = load_dyn(sim, categ, zonmean=zonmean, timemean=timemean, try_load=try_load, model=model, yr_span=yr_span)
        ...
        [dyn, _, _, _, _, _] = load_dyn(sim, categ, zonmean=zonmean, timemean=timemean, try_load=try_load, model=model, yr_span=yr_span)
        ...
        [dyn_ref, _, _, _, _, _] = load_dyn(sim_ref, categ, zonmean=zonmean, timemean=timemean_ref, try_load=try_load, model=model, yr_span=yr_span_ref)
    ...
    ax[1].set_yticks(np.arange(200,1001,200))
    ...
    ax[1].contour(mesh_lat, 1e-2*mesh_lev, 1e-10*np.transpose(circ_ref['psi']), np.arange(vmin_ref, vmax_ref+1, vint_ref), colors='k', linewidths=0.5)
    make_title_sim_time_seas(ax[0], sim, model=modelstr, timemean=timemean, seasmean=seas)
    ax00.set_ylim([-3.3,3.3])
