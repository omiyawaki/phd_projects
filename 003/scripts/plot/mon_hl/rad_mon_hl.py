...
def rad_mon_hl(sim, **kwargs):
    ...
    ############################################
    # PLOT (JR18 prediction, Ra, DEVIATION FROM INITIAL)
    ############################################
    plotname = remove_repdots('%s/rad_jr18_dev_mon_hl.%g.%g.%s' % (plotdir, latbnd[0], latbnd[1], timemean))
    fig, ax = plt.subplots()
    ax.axhline(0, color='k', linewidth=0.5)
    ...
    ############################################
    # PLOT (JR18 prediction, Ra_cs, DEVIATION FROM INITIAL)
    ############################################
    plotname = remove_repdots('%s/radcs_jr18_dev_mon_hl.%g.%g.%s' % (plotdir, latbnd[0], latbnd[1], timemean))
    fig, ax = plt.subplots()
    ax.axhline(0, color='k', linewidth=0.5)
