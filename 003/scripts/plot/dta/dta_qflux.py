...
clf=ax.contourf(lat, plev, np.transpose(dt20), np.arange(-30,31,1), cmap='RdBu_r', vmin=-30, vmax=30, extend='both')
ax.contour(lat, plev, np.transpose(dt20), np.arange(-40,41,1), colors='w', linewidths=0.1)
cb=plt.colorbar(clf)
cb.set_ticks(np.arange(-30,31,10))
cb.set_label(r'$\Delta T$ (K)')
ax.set_xticks(np.arange(-90,91,30))
ax.set_yticks(np.arange(0,1001,200))
ax.set_xlabel('Latitude (deg)')
ax.set_ylabel(r'p (hPa)')
ax.set_title('SSP585 AQUAnoice, DJF')
