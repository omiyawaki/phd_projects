...
def plot_raw2d_lat(sim, varname, **kwargs):
	...
	# select seasons
	if seasmean=='djf':
		var2d = np.roll(file.variables[varname][:],1,axis=0)[:3,:,:]
	elif seasmean=='jja':
		var2d = file.variables[varname][:][5:8,:,:]
	elif seasmean=='':
		var2d = file.variables[varname][:]
	...
	plotname = '%s/%s_lat.%s.%s' % (plotdir, varname, timemean, seasmean)
	fig, ax = plt.subplots()
	...
	ax.set_xlim([-90,90])
	make_title_sim_time_seas(ax, sim, model=model, timemean=timemean, seasmean=seasmean)
