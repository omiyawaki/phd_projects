...
def dmse92500_lat_lon(sim, **kwargs):
    ...
    file_mse = Dataset('%s/mse92500_sm_Amon_bcc-csm1-1_hist+rcp85_r1i1p1_186001-229912.djfmean.nc' % (prefix), 'r')                                       
    mse = np.squeeze(file_mse.variables['mse_sm'][:]) # (time x lat x lon)
    lat = np.squeeze(file_mse.variables['lat'][:]) # (lat)
    lon = np.squeeze(file_mse.variables['lon'][:]) # (lon)

    ############################################
    # Take difference from 1975-2005 mean climatology
    ############################################
    print(yr_base)
    yrstart = 1975-yr_base
    yrend = 2005-yr_base
    meanmse = np.nanmean(mse[yrstart:yrend+1,...], axis=0, keepdims=True)
    dmse = mse - meanmse

    ############################################
    # PLOT (difference at year 2050)
    ############################################
    plotdir = get_plotdir(sim, model=model, yr_span=yr_span, categ=categ)
    [mesh_lat, mesh_lon] = np.meshgrid(lat, lon) # create mesh

    yr_vec = [2050, 2100, 2150, 2200]
    
    for yr in yr_vec:
        iyr = yr - yr_base
        plotname = '%s/dmse92500_lat_lon_%g.%s' % (plotdir, yr, timemean)

        fig, ax = plt.subplots()
        csf = plt.contourf(mesh_lon, mesh_lat, 1e-3*np.transpose(np.squeeze(dmse[iyr,...])))
        plt.title(r'$\Delta m_{925\,\mathrm{hPa}}$ [J g$^{-1}$], %g' % (yr))
