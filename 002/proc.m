...
function proc_flux(type, par)
    ...
    if any(strcmp(type, {'era5', 'erai'}))
        ...
        for fn = fieldnames(ceres.rad)'; fname = fn{1}; % interpolate to std lat and ERA lon
            ...
            ceres.(fname) = interp1(ceres.grid.dim2.lat, ceres.(fname), par.lat_std, 'linear');
    ...
    for fn = {'ra', 'stf', 'res', 'r1', 'r2'}; fname = fn{1};
        if any(strcmp(type, {'era5', 'erai'})); f_vec = par.era.fw;
        elseif strcmp(type, 'gcm'); f_vec = par.gcm.fw; end
        for f = f_vec; fw = f{1};
            for l = {'lo', 'l', 'o'}; land = l{1};
...
function proc_temp_mon_lat(type, par)
    ...
    if strcmp(type, 'era5') | strcmp(type, 'erai')
        ...
        ts = permute(srfc.ts, [2 1 3]); % bring lat to front to interpolate
    ...
    else save(printname, 'ta', 'taz', 'tasi', 'lat', 'tsurf', 'psurf', 'zsurf'); end
    ...
    else save(printname, 'ta_t', 'taz_t', 'tasi_t', 'lat', 'tsurf_t', 'psurf_t', 'zsurf_t'); end
        ...
        if strcmp(type, 'era5') | strcmp(type, 'erai')
            ...
            for ilat = 1:length(lat);
                ...
                for imon = 1:12;
                    ...
                    ma.(land).ta(ilat, imon, :) = calc_ma_dew(ima, grid.dim3.plev, par); % compute moist adiabat with RH
                    maz.(land).ta(ilat, imon, :) = calc_maz_dew(ima, grid.dim3.z, par); % compute moist adiabat with RH
                end
                for ilon = 1:length(grid.dim3.lon);
                    for fn_var = fieldnames(srfc)'
                        ima_t.(fn_var{1}) = ma_t.(land).(fn_var{1})(ilon, ilat);
                    end
                    ma_t.(land).ta(ilat, imon, :) = calc_ma_dew(ima_t, grid.dim3.plev, par); % compute moist adiabat with RH
                    maz_t.(land).ta(ilat, imon, :) = calc_maz_dew(ima_t, grid.dim3.z, par); % compute moist adiabat with RH
                end
            end
        elseif strcmp(type, 'gcm')
            pb = CmdLineProgressBar("Calculating moist adiabats...");
            ...
            for l = {'lo', 'l', 'o'}; land = l{1};
                for t = {'ann', 'djf', 'jja', 'mam', 'son'}; time = t{1};
                    for re = {'rce', 'rae'}; regime = re{1};
                        ...
                        if strcmp(type, 'era5') | strcmp(type, 'erai')
                            ma.rce.all.(fw).(crit).(land).(time).ta = calc_ma_dew(ma.rce.all.(fw).(crit).(land).(time), grid.dim3.plev, par); % compute moist adiabat with dew point temperature
                            ma.rce.tp.(fw).(crit).(land).(time).ta = calc_ma_dew(ma.rce.tp.(fw).(crit).(land).(time), grid.dim3.plev, par); % compute moist adiabat with dew point temperature
                            ma.rce.nh.(fw).(crit).(land).(time).ta = calc_ma_dew(ma.rce.nh.(fw).(crit).(land).(time), grid.dim3.plev, par); % compute moist adiabat with dew point temperature
                            ma.rce.sh.(fw).(crit).(land).(time).ta = calc_ma_dew(ma.rce.sh.(fw).(crit).(land).(time), grid.dim3.plev, par); % compute moist adiabat with dew point temperature
                        elseif strcmp(type, 'gcm')
...
function ma_out = calc_ma_dew(ma_in, plev, par) % compute moist adiabat based on dew point temperature
    if isnan(ma_in.sp) | isnan(ma_in.t2m) | isnan(ma_in.d2m)
        ma_out = nan(size(plev)); % if any of the initial values are nan, ouput nan profile
    else
        pa_cd(1) = ma_in.sp; % set initial pressure
        ta_cd(1) = ma_in.t2m; % set initial temperature
...
function ma_out = calc_maz_dew(ma_in, z_int, par)
