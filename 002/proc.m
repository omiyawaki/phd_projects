...
function proc_flux(type, par)
    ...
    for f = f_vec; fw = f{1};
        if any(strcmp(type, {'era5', 'erai'}));
            ...
            if contains(fw, 'erarad'); flux.ra.(fw) = flux.tsr - flux.ssr + flux.ttr - flux.str; % compute net radiative cooling from radiative fluxes
            else; flux.ra.(fw) = ceres.ra; end % use radiative cooling from CERES data
        elseif strcmp(type, 'gcm');
        ...
        elseif strcmp(type, 'echam');
            flux.toa = flux.trad0 + flux.srad0; % net flux at TOA
            flux.olr = flux.trad0;
    ...
    if strcmp(type, 'era5') | strcmp(type, 'erai')
        for fn = {'sshf', 'slhf', 'cp', 'lsp', 'e', 'lw', 'sw', 'rtoa', 'olr', 'lwsfc', 'swsfc', 'tend', 'divt', 'divg', 'divq', 'TETEN', 'TEDIV', 'don79div'}; fname = fn{1};
        ...
        foldername = sprintf('/project2/tas1/miyawaki/projects/002/data/proc/%s/%s/', type, par.lat_interp);
    elseif strcmp(type, 'gcm')
        for fn = {'hfls', 'hfss', 'prc', 'pr', 'evspsbl', 'lw', 'sw', 'rtoa', 'olr', 'lwsfc', 'swsfc'}; fname = fn{1};
            for l = {'lo', 'l', 'o'}; land = l{1};
                if strcmp(land, 'lo'); flux_n.(land).(fname) = flux.(fname);
                elseif strcmp(land, 'l'); flux_n.(land).(fname) = flux.(fname) .*mask.ocean;
                elseif strcmp(land, 'o'); flux_n.(land).(fname) = flux.(fname) .*mask.land;
        ...
        foldername = sprintf('/project2/tas1/miyawaki/projects/002/data/proc/%s/%s/%s/%s/', type, par.model, par.gcm.clim, par.lat_interp);
    elseif strcmp(type, 'echam')
        for fn = {'ahfl', 'ahfs', 'aprc', 'aprl', 'evap', 'lw', 'sw', 'rtoa', 'olr', 'lwsfc', 'swsfc'}; fname = fn{1};
            for l = {'lo', 'l', 'o'}; land = l{1};
                if strcmp(land, 'lo'); flux_n.(land).(fname) = flux.(fname);
                elseif strcmp(land, 'l'); flux_n.(land).(fname) = flux.(fname) .*mask.ocean;
                elseif strcmp(land, 'o'); flux_n.(land).(fname) = flux.(fname) .*mask.land;
    ...
    for fn = {'ra', 'stf', 'res', 'r1', 'r2', 'ftoa', 'fsfc', 'sfc', 'shf'}; fname = fn{1};
        if any(strcmp(type, {'era5', 'erai'})); f_vec = par.era.fw;
        elseif strcmp(type, 'gcm'); f_vec = par.gcm.fw;
        elseif strcmp(type, 'echam'); f_vec = par.echam.fw; end
        for f = f_vec; fw = f{1};
    ...
    for lo=1:size(pa,2)
        ...
        for la=1:size(pa,3)
            for mo=1:size(pa,4)
                ...
                % add surface data
                if strcmp(type, 'era5') | strcmp(type, 'erai')
                    ta_si(1,lo,la,mo) = srfc.t2m(lo,la,mo);
                elseif strcmp(type, 'gcm')
                    ta_si(1,lo,la,mo) = srfc.tas(lo,la,mo);
                elseif contains(type, 'echam')
                    ta_si(1,lo,la,mo) = srfc.temp2(lo,la,mo);
                end
...
function proc_ta_si(type, par)
    ...
    load(sprintf('%s/srfc.mat', prefix)); % load surface data
    load(sprintf('%s/ta_si.mat', prefix)); % load temperature in sigma
    load(sprintf('%s/%s/masks.mat', prefix_proc, par.lat_interp)); % load land and ocean masks
                        ...
                        for d = {'all', 'nh', 'sh', 'tp'}; domain = d{1};
                            if strcmp(regime, 'rce')
                                ...
                                elseif strcmp(domain, 'nh')
                                    ...
                                    denm = ta_si_t.(land).(time).(fw).(crit).(regime)(:,lat>30,:);
...
function ma_out = calc_ma_dew_si(ma_in, plev, par, type, grid)
    ...
    if isnan(ps) | isnan(tas) | isnan(tdas)
        ...
        ma_si = -1e3*interp1(real(pa_s)/ps, real(ta_s), grid.dim3.si, 'spline');
...
function ma_out = calc_maz_dew(ma_in, z_int, par, type)
    ...
    if isnan(ps) | isnan(tas) | isnan(tdas)
        ...
        pa_s = interp1(z_s, pa_s, z_int, 'spline');
        ta_s = interp1(z_s, ta_s, z_int, 'spline');
        qsat_s = interp1(z_s, qsat_s, z_int, 'spline');
        dtadz_s = interp1(z_s, dtadz_s, z_int, 'spline');
...
function ma_out = calc_maz_dew_pa(ma_in, plev, par, type)
    ...
    if isnan(ps) | isnan(tas) | isnan(tdas)
        ...
        dtadz_s = -1e3*interp1(real(pa_s), real(dtadz_s), plev, 'spline'); % output in K/km at standard pressure grid
...
function ma_out = calc_maz_dew_si(ma_in, plev, par, type, grid)
    ...
    if isnan(ps) | isnan(tas) | isnan(tdas)
        ...
        dtadz_s = interp1(real(pa_s)/ps, real(dtadz_s), grid.dim3.si, 'spline');
...
function ma_out = calc_ma_hurs_si(ma_in, plev, par, grid)
    if isnan(ma_in.ps) | isnan(ma_in.tas) | isnan(ma_in.hurs) | isnan(ma_in.zs)
        ...
        ma_si = interp1(real(pa_s)/ma_in.ps, real(ta_s), grid.dim3.si, 'spline');
...
function ma_out = calc_maz_hurs_si(ma_in, plev, par, grid)
    if isnan(ma_in.ps) | isnan(ma_in.tas) | isnan(ma_in.hurs) | isnan(ma_in.zs)
        ...
        dtadz_s = -1e3*interp1(real(pa_s)/ma_in.ps, real(dtadz_s), grid.dim3.si, 'spline'); % output in K/km at standard pressure grid
