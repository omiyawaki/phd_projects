function ma_out = calc_ma_q_si(ma_in, plev, par, type, grid)
    if strcmp(type, 'merra2')
        ps = ma_in.PS; tas = ma_in.T2M; qs = ma_in.QV2M;
    end

    if isnan(ps) | isnan(tas) | isnan(qs)
    ...
    if isfield(ma_in, 'pinit')
        ...
        e_cd(1) = calc_e(ps, qs, par); % calculate initial vapor pressure
