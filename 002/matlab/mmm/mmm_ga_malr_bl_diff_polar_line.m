function mmm_ga_malr_bl_diff_polar_line(type, par)
    lat = par.lat;
    ...
    for lb = 1:length(lat_bound_list); par.lat_bound = lat_bound_list(lb);
        ...
        for k=1:length(par.model_list); par.model = par.model_list{k};
            ...
            ga_frac_lat0 = load(sprintf('%s/si_bl_%g/ga_malr_bl_diff_poleward_of_lat_%g.mat', prefix_proc, par.si_bl, par.lat_bound));
        ...
        save(sprintf('%sga_malr_bl_diff_poleward_of_lat_%g.mat', foldername, par.lat_bound), 'ga_frac_lat_mmm', 'ga_frac_lat_std', 'ga_frac_lat_min', 'ga_frac_lat_max', 'ga_frac_lat_25', 'ga_frac_lat_75', 'lat', '-v7.3');
