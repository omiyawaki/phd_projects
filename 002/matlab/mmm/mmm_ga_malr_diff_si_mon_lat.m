function mmm_ga_malr_diff_si_mon_lat(type, par)
    ...
    for k=1:length(par.model_list); par.model = par.model_list{k};
        ...
        ga_malr_diff0 = load(sprintf('%s/si_bl_%g/ga_malr_diff_si_mon_lat.mat', prefix_proc, par.si_bl));
    ...
    save(sprintf('%s%s', foldername, 'ga_malr_diff_si_mon_lat'), 'ga_malr_diff', 'lat', '-v7.3');
