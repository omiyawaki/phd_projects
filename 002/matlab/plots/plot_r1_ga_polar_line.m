            ...
            for f = par.(type).fw; fw = f{1};
                ...
                for l = par.land_list; land = l{1};
                    ...
                    for lb = 1:length(lat_list); par.lat_bound = lat_list(lb);
                        ...
                        if par.make_tikz & par.lat_bound > 0 & par.si_bl == 0.9 & par.si_up == 0.3 & strcmp(fw, 'mse_old')
                        ...
                        make_title_type_lat(type, lat1, lat2, par);
                        set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
                        print(sprintf('%s/ga_malr_diff/si_bl_%g/%s/%s/r1_gablft_mon_clean_%g_%s_%s.png', plotdir, par.si_bl, fw, land, par.si_up, hemi, lat_print), '-dpng', '-r300');
                        if par.make_tikz & par.lat_bound > 0 & par.si_bl == 0.9 & par.si_up == 0.3 & strcmp(fw, 'mse_old')
                            matlab2tikz(sprintf('%s/ga_malr_diff/si_bl_%g/%s/%s/r1_gablft_mon_clean_%g_%s_%s.tex', plotdir, par.si_bl, fw, land, par.si_up, hemi, lat_print));
                        end
                        close;
