        ...
        for ga = 1:length(par.ga_swp); par.ga = par.ga_swp(ga);
            ...
            for f = par.(type).fw; fw = f{1};
                ...
                % for l = {'lo'}; land = l{1};
                    ...
                    %         mse_r1 = flux_z.(land).res.mse(:,mon_eval)./flux_z.(land).ra.mse(:,mon_eval);
                    %     end
                    ...
                    for lb = 1:length(lat_list); par.lat_bound = lat_list(lb);
                        ...
                        r1 = interp1(grid.dim2.lat, flux_z.(land).res.(fw)./flux_z.(land).ra.(fw), lat);
                        ...
                        if ~strcmp(type, 'hahn')
                            if strcmp(fw, 'mse_old')
                                mse_r1 = interp1(grid.dim2.lat, flux_z.(land).res.mse./flux_z.(land).ra.mse, lat);
                                mse_r1 = nansum(clat_mon .* mse_r1)/nansum(clat); % area averaged weighting
                            end
                        end
                        ...
                        set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
                        set(gca, 'fontsize', par.fs, 'xlim', [1 12], 'xtick', [1:12], 'xticklabel', monlabel, 'yminortick', 'on', 'ylim', [-40 80]) 
                        ax = gca;
                        ax.YAxis(1).Color = 'k';
                        ax.YAxis(2).Color = par.green;
                        print(sprintf('%s/ga_malr_diff/si_bl_%g/%s/%s/r1_ga_mon_%g_%s_%s.png', plotdir, par.si_bl, fw, land, par.si_up, hemi, lat_print), '-dpng', '-r300');
                        close;
                        ...
                        if abs(par.lat_bound) > 45
                            ...
                            %     set(gca, 'fontsize', par.fs, 'xlim', [1 12], 'xtick', [1:12], 'xticklabel', monlabel, 'yminortick', 'on', 'ylim', [-50 400]) 
                            % else
