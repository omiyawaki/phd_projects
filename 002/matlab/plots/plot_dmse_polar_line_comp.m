function plot_dmse_polar_line_comp(type, par)
    make_dirs(type, par)
    ...
    for lb = 1:length(lat_bound_list); par.lat_bound = lat_bound_list(lb);
        ...
        tmp = load(sprintf('%s/dmse_poleward_of_lat_%g', prefix_proc_era5c, par.lat_bound)); dmse_era5c = tmp.dmse; clear tmp;
        tmp = load(sprintf('%s/dmse_poleward_of_lat_%g', prefix_proc_merra, par.lat_bound)); dmse_merra = tmp.dmse; clear tmp;
        tmp = load(sprintf('%s/dmse_poleward_of_lat_%g', prefix_proc_jra55, par.lat_bound)); dmse_jra55 = tmp.dmse; clear tmp;
        ...
        for l = {'lo'}; land = l{1};
            ...
            for f = f_vec; fw = f{1};
                ...
                par.folder = sprintf('%s/dmse-comp/%s/%s/0_poleward_of_lat_%g', plotdir, fw, land, par.lat_bound);
