function plot_dmse_midlatitude_line(type, par)
    ...
    for l = {'lo'}; land = l{1};
        ...
        for f = f_vec; fw = f{1};
            for lb = 1:length(lat_bound_list); lat_bound = lat_bound_list(lb);
                ...
                folder = sprintf('%s/dmse/%s/%s/0_midlatitude_lat_%g_to_%g', plotdir, fw, land, lat_center-lat_bound, lat_center+lat_bound);
                ...
                % xlabel('Month');
                ylabel(sprintf('Energy flux (Wm$^{-2}$)'));
                legend([dra dres dstf], '$\Delta R_a$', '$\Delta(\nabla\cdot F_m)$', '$\Delta (\mathrm{LH+SH})$', 'location', 'eastoutside', 'orientation', 'vertical');
                set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos_verywide)
    ...
    %         folder = sprintf('%s/dmse/%s/0_midlatitude_pm_lat_%g', plotdir, fw, lat_bound);
    %         if ~exist(folder, 'dir'); mkdir(folder); end;
