    ...
    for f = {'mse_old'}; fw = f{1};
        ...
        for l = {'lo'}; land = l{1};
            ...
            for lb = 1:length(lat_list); lat_eval = lat_list(lb);
                ...
                ga = interp1(grid.dim3.lat, ga_diff.(land), lat_eval);
                ga_mon = squeeze(ga);
                ga_fr = interp1(grid.dim3.lat, ga_frac.(land), lat_eval);
                ga_fr_mon = squeeze(ga_fr);
                gad_fr = interp1(grid.dim3.lat, gad_frac.(land), lat_eval);
                ...
                set(gca, 'fontsize', par.fs, 'ydir', 'reverse', 'yscale', 'linear', 'ytick', [0:0.1:1], 'ylim', [0.2 1], 'xminortick', 'on')
                ...
                make_title_type_lat_pt(type, lat_eval, par);
                set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
                set(gca, 'fontsize', par.fs, 'xlim', [-2 6], 'ydir', 'reverse', 'yscale', 'linear', 'ytick', [0:0.1:1], 'ylim', [0.2 1], 'xminortick', 'on')
                ...
                for mon = 1:6
                    ...
                    if abs(lat_eval) < 60
                        text(ga_fr_mon(mon,mon*10)-0.01, grid.dim3.si(mon*10), num2str(mon), 'color', cmp(idxbin, :), 'fontsize', 6);
                    else
                        text(ga_fr_mon(mon,mon*3)-0.01, grid.dim3.si(mon*3), num2str(mon), 'color', cmp(idxbin, :), 'fontsize', 6);
                    end
                end
                ...
                for mon = 7:12
                    [~, idxbin] = min(abs(r1(mon) - r1_bins_fine));
                    plot(ga_fr_mon(mon,:), grid.dim3.si, 'color', cmp(idxbin,:), 'linewidth', 0.5);
                    text(ga_fr_mon(mon,(mon-6)*10)-0.01, grid.dim3.si((mon-6)*10), num2str(mon), 'color', cmp(idxbin, :), 'fontsize', 6);
                end
                xlabel('$\frac{\Gamma_m - \Gamma}{\Gamma_m}$ (\%)'); ylabel('$\sigma$ (unitless)');
                axis('tight');
                caxis([min(par.r1_bins_45) max(par.r1_bins_45)]);
                ...
                for mon = 1:6
                    [~, idxbin] = min(abs(r1(mon) - r1_bins_fine));
                    plot(gad_fr_mon(mon,:), grid.dim3.si, 'color', cmp(idxbin,:), 'linewidth', 0.5);
                    if abs(lat_eval) < 60
                        text(gad_fr_mon(mon,mon*10)-0.01, grid.dim3.si(mon*10), num2str(mon), 'color', cmp(idxbin, :), 'fontsize', 6);
                    else
                        text(gad_fr_mon(mon,mon*3)-0.01, grid.dim3.si(mon*3), num2str(mon), 'color', cmp(idxbin, :), 'fontsize', 6);
                    end
                end
                xlabel('$\frac{\Gamma_m - \Gamma}{\Gamma_m}$ (\%)'); ylabel('$\sigma$ (unitless)');
                axis('tight');
                ...
                set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
                set(gca, 'fontsize', par.fs, 'ydir', 'reverse', 'yscale', 'linear', 'ytick', [0:0.1:1], 'ylim', [0.2 1], 'xminortick', 'on')
                print(sprintf('%s/gad_fr_r1_mon_1_6.png', folder), '-dpng', '-r300');
                close;
                ...
                for mon = 7:12
                    [~, idxbin] = min(abs(r1(mon) - r1_bins_fine));
                    plot(gad_fr_mon(mon,:), grid.dim3.si, 'color', cmp(idxbin,:), 'linewidth', 0.5);
                    text(gad_fr_mon(mon,(mon-6)*10)-0.01, grid.dim3.si((mon-6)*10), num2str(mon), 'color', cmp(idxbin, :), 'fontsize', 6);
                end
                xlabel('$\frac{\Gamma_m - \Gamma}{\Gamma_m}$ (\%)'); ylabel('$\sigma$ (unitless)');
                axis('tight');
                caxis([min(par.r1_bins_45) max(par.r1_bins_45)]);
                ...
                set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
                set(gca, 'fontsize', par.fs, 'ydir', 'reverse', 'yscale', 'linear', 'ytick', [0:0.1:1], 'ylim', [0.2 1], 'xminortick', 'on')
                print(sprintf('%s/gad_fr_r1_mon_7_12.png', folder), '-dpng', '-r300');
                close;
