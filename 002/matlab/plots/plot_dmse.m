...
function plot_dmse(ra_lat, res_lat, lh_lat, sh_lat, tend_lat, leg, ymin, ymax, type, fw, par, ra_std, res_std, lh_std, sh_std, tend_std, ra_std2, res_std2, lh_std2, sh_std2, tend_std2)
    ...
    if strcmp(fw, 'ceresrad')
        ...
        chf=plot([1:12], circshift(lh_lat+sh_lat,par.shiftby,2),':', 'color', par.orange);
        chf=plot([1:12], circshift(lh_lat+sh_lat,par.shiftby,2),'--', 'color', par.blue);
    end
    ...
    %         ch_std2 = [circshift(lh_lat+lh_std+sh_lat+sh_std, par.shiftby, 2), fliplr(circshift(lh_lat-lh_std+sh_lat-sh_std, par.shiftby, 2))];
    ...
    %         ch_std2 = [circshift(ch_std2, par.shiftby, 2), fliplr(circshift(ch_std, par.shiftby, 2))];
    %     end
    ...
    %     fill(mon2, ch_std2, par.orange, 'facealpha', 0.2, 'edgealpha', 0.2);
    % end
    ...
    if leg == ""
        ...
        %         legend([ra res chf], '$Radiative$', 'Advective', 'Convective', 'location', 'eastoutside', 'orientation', 'vertical');
        %     end
        % end
