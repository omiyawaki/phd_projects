function plot_dthedz_binned_r1(type, par)
    make_dirs(type, par)
    ...
    for f = par.(type).fw; fw = f{1};
        for l = par.land_list; land = l{1};
            ...
            eptlr = reshape(dthedz.(land), [], length(grid.dim3.si)); % multiply by 100 to express in percentage
            ...
            xlabel('$\frac{\partial \theta_e}{\partial z}$ (K km$^{-1}$)'); ylabel('$\sigma$ (unitless)');
            axis('tight');
            ...
            print(sprintf('%s/eptlr_binned_r1/%s/%s/eptlr_r1_all.png', plotdir, fw, land), '-dpng', '-r300');
            close;
            ...
            set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos_wide)
            set(gca, 'fontsize', par.fs, 'ydir', 'reverse', 'yscale', 'linear', 'ytick', [0:0.1:1], 'ylim', [0.3 1], 'xminortick', 'on', 'xlim', [-10 10], 'yminortick', 'on')
            print(sprintf('%s/eptlr_binned_r1/%s/%s/eptlr_r1_all_nocb.png', plotdir, fw, land), '-dpng', '-r300');
            close;
