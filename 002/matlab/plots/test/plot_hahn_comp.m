function plot_hahn_comp(type, par)
    
    if ~strcmp(par.hahn.clim, 'Control1850')
        error('This analysis must be run with the Hahn Control1850 setting.')
    end

    make_dirs(type, par)

    % LOAD DATA WITH TOPOGRAPHY SIM
    prefix = make_prefix(type, par);
    prefix_proc = make_prefix_proc(type, par);
    plotdir = make_plotdir(type, par);
    % LOAD FLAT DATA (make sure to run this with control1850)
    par2 = par;
    par2.hahn.clim = 'Flat1850';
    ...
    for lb = 1:length(lat_bound_list); par.lat_bound = lat_bound_list(lb);
        ...
        for l = par.land_list; land = l{1};
            ...
            title(sprintf('%s, $\\phi=%g^\\circ$ to $%g^\\circ$', upper('CESM'), par.lat_bound, par.lat_pole));
            ...
            plotname = sprintf('%s/0_mon_r1z_hahn_comp', par.folder);
            ...
            set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [ylim_lo ylim_up], 'yminortick', 'on', 'tickdir', 'out');
            plotname = sprintf('%s/0_mon_ra_hahn_comp', par.folder);
            print(plotname, '-dpng', '-r300');
            close;
            ...
            set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos)
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [ylim_lo ylim_up], 'yminortick', 'on', 'tickdir', 'out');
            plotname = sprintf('%s/0_mon_res_hahn_comp', par.folder);
            print(plotname, '-dpng', '-r300');
            close;
            ...
            lh = interp1(grid.dim2.lat, flux_z.(land).LHFLX, lat);
            lh_lat = nansum(clat_mon.*lh)/nansum(clat);

            lh2 = interp1(grid.dim2.lat, flux_z2.(land).LHFLX, lat);
            lh_lat2 = nansum(clat_mon.*lh2)/nansum(clat);

            sh = interp1(grid.dim2.lat, flux_z.(land).SHFLX, lat);
            sh_lat = nansum(clat_mon.*sh)/nansum(clat);

            sh2 = interp1(grid.dim2.lat, flux_z2.(land).SHFLX, lat);
            ...
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [ylim_lo ylim_up], 'yminortick', 'on', 'tickdir', 'out');
            plotname = sprintf('%s/0_mon_dflux_hahn_comp', par.folder);
            print(plotname, '-dpng', '-r300');
            close;
            ...
            set(gcf, 'paperunits', 'inches', 'paperposition', par.ppos_verywide)
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [ylim_lo ylim_up], 'yminortick', 'on', 'tickdir', 'out');
            plotname = sprintf('%s/0_mon_decompr1z_hahn_comp', par.folder);
            print(plotname, '-dpng', '-r300');
            close;
