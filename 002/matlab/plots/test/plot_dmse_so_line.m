function plot_dmse_so_line(type, par)
    make_dirs(type, par)
    ...
    for l = {'lo'}; land = l{1};
        ...
        for f = f_vec; fw = f{1};
            for lb = 1:length(lat_center_list); lat_center = lat_center_list(lb);
                dlat = 0.25; % step size for standard lat grid
                if lat_center>0; lat = [-lat_bound:dlat:lat_bound]+lat_center; shiftby=0; monlabel=par.monlabel;
                else; lat = [-lat_bound:dlat:lat_bound]+lat_center; shiftby=6; monlabel=par.monlabelsh; end;
                clat = cosd(lat); % cosine of latitude for cosine weighting
                clat_mon = repmat(clat', [1 12]);

                folder = sprintf('%s/dmse/%s/%s/0_so_pm_lat_%g', par.plotdir, fw, land, lat_center-lat_bound);
                ...
                if any(strcmp(type, {'era5', 'era5c', 'erai'})); title(sprintf('%s, %s, $\\phi=%g^\\circ$ to $%g^\\circ$', upper(type), land_text, lat_center-lat_bound, lat_center+lat_bound));
