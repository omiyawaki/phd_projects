function plot_flux_comp(type, par)
    make_dirs(type, par)
    ...
    prefix_era5 = make_prefix('era5c', par);
    prefix_proc_era5 = make_prefix_proc('era5c', par);
    ...
    tmp = load(sprintf('%s/grid.mat', prefix_era5)); grid_era5 = tmp.grid; clear tmp; % read grid data
    tmp = load(sprintf('%s/flux_z.mat', prefix_proc_era5)); flux_z_era5 = tmp.flux_z; clear tmp; % load lat x mon RCAE data
    tmp = load(sprintf('%s/flux_t.mat', prefix_proc_era5)); flux_t_era5 = tmp.flux_t; clear tmp; % load lat x lon RCAE data
    ...
    if strcmp(type, 'gcm') & ~strcmp(par.gcm.clim, 'hist-pi') & ~contains(par.model, 'mmm')
    ...
    for l = {'lo'}; land = l{1};
        ...
        for f = f_vec; fw = f{1};
            ...
            cmp = colCog(30);
            colormap(cmp);
            contourf(mesh_lat, mesh_mon, interp1(grid_era5.dim3.lat, flux_z_era5.(land).res.(fw), lat)-flux_z.(land).res.(fw), [-100 -50 -30:2:30 50 100], 'linecolor', 'none');
            title('$\mathrm{ERA5 - CMIP5\,mmm}$');
            caxis([-30 30]);
            cb = colorbar('limits', [-30 30], 'ytick', [-30:10:30], 'location', 'eastoutside');
            ...
            print(sprintf('%s/flux-comp/%s/%s/0_div_mon_lat', plotdir, fw, land), '-dpng', '-r300');
            close;

            % lat x mon dependence of RCE and RAE
            if strcmp(fw, 'dse'); var_text = '$\nabla \cdot F_s$';
            else var_text = '$\Delta R_a$'; end
            figure(); clf; hold all; box on;
            cmp = colCog(30);
            colormap(cmp);
            contourf(mesh_lat, mesh_mon, interp1(grid_era5.dim3.lat, flux_z_era5.(land).ra.(fw), lat)-flux_z.(land).ra.(fw), [-100 -50 -30:2:30 50 100], 'linecolor', 'none');
            title('$\mathrm{ERA5 - CMIP5\,mmm}$');
            caxis([-30 30]);
            cb = colorbar('limits', [-30 30], 'ytick', [-30:10:30], 'location', 'eastoutside');
            cb.TickLabelInterpreter = 'latex'; cb.Label.Interpreter = 'latex';
            ...
            xlabel('Month'); ylabel('Latitude (deg)');
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [-90 90], 'ytick', [-90:30:90], 'yminortick', 'on', 'tickdir', 'out');
            print(sprintf('%s/flux-comp/%s/%s/0_ra_mon_lat', plotdir, fw, land), '-dpng', '-r300');
            close;

            % lat x mon dependence of RCE and RAE
            if strcmp(fw, 'dse'); var_text = '$\nabla \cdot F_s$';
            else var_text = '$\Delta$LH'; end
            figure(); clf; hold all; box on;
            cmp = colCog(30);
            colormap(cmp);
            contourf(mesh_lat, mesh_mon, interp1(grid_era5.dim3.lat, -flux_z_era5.(land).slhf, lat)-flux_z.(land).hfls, [-100 -50 -30:2:30 50 100], 'linecolor', 'none');
            title('$\mathrm{ERA5 - CMIP5\,mmm}$');
            caxis([-30 30]);
            cb = colorbar('limits', [-30 30], 'ytick', [-30:10:30], 'location', 'eastoutside');
            cb.TickLabelInterpreter = 'latex'; cb.Label.Interpreter = 'latex';
            ...
            xlabel('Month'); ylabel('Latitude (deg)');
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [-90 90], 'ytick', [-90:30:90], 'yminortick', 'on', 'tickdir', 'out');
            print(sprintf('%s/flux-comp/%s/%s/0_lh_mon_lat', plotdir, fw, land), '-dpng', '-r300');
            close;

            % lat x mon dependence of RCE and RAE
            if strcmp(fw, 'dse'); var_text = '$\nabla \cdot F_s$';
            else var_text = '$\Delta$SH'; end
            figure(); clf; hold all; box on;
            cmp = colCog(30);
            colormap(cmp);
            contourf(mesh_lat, mesh_mon, interp1(grid_era5.dim3.lat, -flux_z_era5.(land).sshf, lat)-flux_z.(land).hfss, [-100 -50 -30:2:30 50 100], 'linecolor', 'none');
            title('$\mathrm{ERA5 - CMIP5\,mmm}$');
            caxis([-30 30]);
            cb = colorbar('limits', [-30 30], 'ytick', [-30:10:30], 'location', 'eastoutside');
            cb.TickLabelInterpreter = 'latex'; cb.Label.Interpreter = 'latex';
            ...
            xlabel('Month'); ylabel('Latitude (deg)');
            set(gca, 'xlim', [1 12], 'xtick', [1:12], 'xticklabels', par.monlabel, 'ylim', [-90 90], 'ytick', [-90:30:90], 'yminortick', 'on', 'tickdir', 'out');
            print(sprintf('%s/flux-comp/%s/%s/0_sh_mon_lat', plotdir, fw, land), '-dpng', '-r300');
            close;
