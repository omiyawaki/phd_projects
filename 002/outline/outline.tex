...
\begin{itemize}
  ...
  \item When we bin the temperature profiles according to $R_{1}$, the near surface inversion gradually weakens with decreasing $R_{1}$ and vanishes around $R_{1}=0.9$ (Fig.~\ref{fig:temp-binned-r1}). For $R_{1}<0.9$, the free tropospheric temperature profile becomes less stable with decreasing $R_{1 }$.
...
\begin{itemize}
  ...
  \item Using the above assumptions, we can write Equation~(\ref{eq:r1-linear2}) as
        ...
        \end{equation}
  \item We now use the solution for $T_{s}^{*}$ and $\Phi$ from the 0-dimensional Budyko-Sellers energy balance model, where
        \begin{align}
...
\begin{figure}
  \noindent\includegraphics[width=\textwidth]{temp-binned-r1.png}\\
  \caption{Temperature profiles binned according to $R_{1}$ for (a) ERA-5 reanalysis and (b) CMIP5 historical multi-model mean. The near surface inversion becomes weaker with decreasing $R_{1}$ (in the direction of increasing temperatures) and vanishes around $R_{1}=0.9$. Beyond this point, the free tropospheric temperature profile becomes less stable with decreasing $R_{1}$.}
