...
for k=1:length(par.echam_clims); par.echam.clim=par.echam_clims{k};
    ...
    for lb = 1:length(lat_bound_list); lat_bound = lat_bound_list(lb);
        ...
        tsz_lat.(par.echam.clim) = interp1(grid.dim2.lat, squeeze(nanmean(srfc.(par.echam.clim).tsurf, 1)), lat);
        tsz_lat.(par.echam.clim) = nansum(tsz_lat.(par.echam.clim).*clat_mon)/nansum(clat);

        % annual mean
        tsz_ann.(par.echam.clim) = repmat(squeeze(nanmean(nanmean(srfc.(par.echam.clim).tsurf,1),3))', [1 12]);
        ...
        % seasonality
        dtsz.(par.echam.clim) = squeeze(nanmean(srfc.(par.echam.clim).tsurf,1)) - tsz_ann.(par.echam.clim);
