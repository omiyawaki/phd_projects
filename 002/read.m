...
par.era.vars.srfc = {'sp', 't2m', 'd2m', 'zs', 'skt'}; % surface variables to read (sp = surface pressure, t2m = 2m temp, d2m = 2m dew point temp)
...
par.merra2c.vars.srfc = {'PS', 'T2M', 'QV2M', 'zs', 'TS'};
...
par.jra55.vars.srfc = {'ps', 'tas', 'hurs', 'zs', 'ts'};
...
function run_func(type, par)
    read_grid(type, 'ymonmean', par) % grid, i.e. lon, lat, plev
    ...
    % make_thetaeqsi(type, par) % compute equivalent potential temperature in sigma coordinates
    % make_msesi(type, par) % compute equivalent potential temperature in sigma coordinates
    ...
    % read_snevap(type, par) % snow evaporation (kg m^-2 s^-1)
    % read_snmelt(type, par) % snow melt (kg m^-2 s^-1)
