...
declare -a vars_gcm=("tendmon") # list of GCM variables that we want to process
...
for dirs in ${models[@]}; do # loop through models
    ...
    *) 
        ...
        for vars in ${vars_gcm[@]}; do
            echo $vars
            if ls $cwd/${dirs}${vars}_*rcp85*.nc 1> /dev/null 2>&1; then # check if data is already there
                ...
                if [ ${#files[@]} -eq 0 ]; then # check if this file exists
                    ...
                    sel="${common}207001-209912" # write files name with last 5 years
                    cdo seldate,2070-01-01,2099-12-31 $files $cwd/$dirs$sel.nc # select last 5 years
                    ...
                    sel="${common}207001-209912" # write file name with selected years
                    ...
                    cdo seldate,2070-01-01,2099-12-31 $cwd/$dirs/$merge.nc $cwd/$dirs/$sel.nc # select years 1979-2005
                    cdo deltat $cwd/$dirs$sel.nc $cwd/$dirs$sel.deltat.nc
                    cdo ymonmean $cwd/$dirs/$sel.deltat.nc $cwd/$dirs/$sel.deltat.ymonmean.nc # take multi-year monthly climatology and save
