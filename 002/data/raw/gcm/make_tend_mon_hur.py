...
hur = file_hur.variables['hur'][:] # (day x lev x lat x lon)
...
if not np.array_equal(lat2d,lat3d_zg):
    ...
    if not np.array_equal(lat3d,lat3d_zg):
        ...
        ta = f(filledlat3d)
        f = interpolate.interp1d(filledlat2d, hur, axis=2)
        hur = f(filledlat3d)
...
ta[idx_below]=np.nan
zg[idx_below]=np.nan
hur[idx_below]=np.nan

# convert hur to hus
esat = 611 * np.exp(6808 * (1/273 - 1/ta) - 5.09 * np.log(ta/273))
e = hur/100 * esat
q = eps * e / pa3d
q[idx_below]=np.nan
