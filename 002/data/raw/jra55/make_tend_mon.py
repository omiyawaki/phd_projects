...
ps = file_ps.variables['PRES_GDS0_SFC_S123'][:] # (mon x lat x lon)
# tas = file_tas.variables['tas'][:] # (mon x lev x lat x lon)
ta = file_ta.variables['TMP_GDS0_ISBL_S123'][:] # (day x lev x lat x lon)
zg = file_zg.variables['HGT_GDS0_ISBL_S123'][:] # (day x lev x lat x lon)
hus = file_hus.variables['SPFH_GDS0_ISBL_S123'][:] # (day x lev x lat x lon)
plev = 100 * file_ta.variables['lv_ISBL1'][:]  # convert hPa to Pa
...
lat2d = file_ps.variables['g0_lat_1'][:] # (mon x lat x lon)
lat3d = file_zg.variables['g0_lat_2'][:] # (day x lev x lat x lon)
lat3d_zg = file_zg.variables['g0_lat_2'][:] # (day x lev x lat x lon)
...
for name, dimension in file_zg.dimensions.items():
    if any(name in s for s in ['lv_ISBL1']):
        continue
...
for name, variable in file_zg.variables.items():
    if any(name in s for s in ['HGT_GDS0_ISBL_S123' 'lv_ISBL1']):
        continue
...
tend = file_tend.createVariable('tend', 'f4', ("initial_time0_hours","g0_lat_2","g0_lon_3"))
tend.units = "W/m^2"
