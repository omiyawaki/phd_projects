...
ps = file_ps.variables['varmo'][:] # (mon x lat x lon)
# tas = file_tas.variables['tas'][:] # (mon x lev x lat x lon)
ta = file_ta.variables['varmo'][:] # (day x lev x lat x lon)
zg = file_zg.variables['varmo'][:] # (day x lev x lat x lon)
hus = file_hus.variables['varmo'][:] # (day x lev x lat x lon)
plev = 100 * file_ta.variables['lev'][:] # multiply by 100 to convert hPa to Pa
...
for name, size in file_zg.dimensions.items():
    if any(name in s for s in ['lev']):
        continue
    file_tend.createDimension(name, len(size) if not size.isunlimited() else None)
 
# copy all variables except time from ps file
for name, variable in file_zg.variables.items():
    if any(name in s for s in ['varmo' 'lev']):
        continue
    
    x = file_tend.createVariable(name, variable.datatype, name)
    file_tend[name].setncatts(file_zg[name].__dict__)
    file_tend[name][:] = file_zg[name][:]
    
tend = file_tend.createVariable('tend', 'f4', ("month","lat","lon"))
tend.units = "W/m^2"
