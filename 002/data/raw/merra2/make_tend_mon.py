...
zg = file_zg.variables['H'][:] # (day x lev x lat x lon)
hus = file_hus.variables['QV'][:] # (day x lev x lat x lon)
plev = 100 * file_ta.variables['lev'][:]  # convert hPa to Pa
...
for name, variable in file_zg.variables.items():
    if any(name in s for s in ['H' 'lev']):
        continue
