...
path_srfc = sys.argv[1]
path_temp = sys.argv[2]
path_tempsi = sys.argv[3]

# open files
file_srfc = Dataset(path_srfc, 'r')
file_temp = Dataset(path_temp, 'r')

# read data
ps = file_srfc.variables['sp'][:] # (mon x lat x lon)
tas = file_srfc.variables['t2m'][:] # (mon x lat x lon)
temp = file_temp.variables['t'][:] # (mon x lev x lat x lon)
plev = 1e2*file_temp.variables['level'][:] # (lev)
...
temp_si = file_tempsi.createVariable('tempsi', 'f4', ("time","level","lat","lon"))
temp_si.units = "K"
