...
Quantifying the latitudinal\added{ and seasonal} dependence of observed energy balance regimes would allow us to assess where\added{ and when} idealized models that assume RCE or RAE hold.\deleted{ This is particularly important for RCE, which has become a standard idealized configuration for tropical theories (e.g., Emanuel and Bister 1996; Nilsson and Emanuel 1999; Romps 2014; Singh and O'Gorman 2015) and simulations (Wing et al. 2018, and the references therein).}\added{ Validating the latitudinal structure and seasonality of RCE would have broad implications due to its widespread use for understanding tropical climate and its response to warming, such as tropical precipitation \citep[e.g.,][]{romps2011,pendergrass2016, abbott2020}, cyclones \citep{nolan2007,khairoutdinov2013,merlis2019}, and climate sensivitity \citep{sarachik1978, renno1994, coppin2018}. Quantifying the latitudinal and seasonal dependence of RCE can guide where and when these insights are applicable in the real world.} 

\added{RCE and RAE used to understand the vertical temperature response to warming in the tropics and high latitudes \citep{manabe1964,held1993,payne2015,cronin2016}. Thus, a quantitative metric of energy balance regimes (RCE, RAE, and RCAE) may be used as a proxy for identifying key features in the lapse rate (moist adiabatic, surface inversion, and mixed) and its response to CO$_2$ forcing.}\deleted{Quantifying the link between energy balance and lapse rate regimes would also allow us to better understand the regional response to CO$_2$ forcing.} For example, regions of RCE that exhibit a moist adiabatic lapse rate will have amplified warming aloft in response to increased CO$_2$ \citep{held1993, romps2011}. In contrast, regions of RAE that exhibit a surface inversion will have amplified warming at the surface in response to increased CO$_2$ \citep{held1993, cronin2016}.\added{ While \cite{payne2015} demonstrate that idealized column models of RCE and RAE predict amplified warming in the upper troposphere and the surface, respectively, the applicability of temperature predictions based on RCE and RAE on latitudinal and seasonal scales is unknown. A quantitative metric for energy balance regimes would make it possible to evaluate whether the connection between energy balance regimes and the vertical temperature structure holds across spatial and temporal scales using comprehensive GCMs.}

Recent work by \cite{jakob2019} quantified regions of RCE using the dry static energy (DSE) budget and a dimensional threshold (sum of radiative cooling, latent heating associated with precipitation, and surface sensible heating $< \pm 50$ Wm$^{-2}$). They find that RCE is approximately satisfied over large spatial ($>5000$ km) and temporal ($>$ daily) scales in the tropics. However, \replaced{we still lack a complete picture of the latitudinal and seasonal structure of energy balance regimes, such as the applicability of}{the latitudinal distribution of} RCE outside the tropics and the distribution of RAE and RCAE \replaced{regimes}{have not been investigated}.

\replaced{Here, we seek to answer the questions}{We therefore seek to answer the questions}: \replaced{where and when}{when and where} do energy balance regimes (RCE, RAE\deleted{, and RCAE}) occur on modern Earth, and how closely are they linked to lapse rate regimes (moist adiabatic and surface inversion)? To answer these questions, we develop a \replaced{nondimensional number based on the MSE budget to quantify}{quantitative definition for} energy balance regimes\deleted{ using the nondimensionalized MSE budget} (Section~\ref{sec:methods}\ref{subsec:mse}). We use this \replaced{nondimensional number to quantify}{definition to quantify} where and when energy balance regimes occur in \replaced{latitudinally}{the annual mean} and seasonally on modern Earth using reanalysis and CMIP5 data. We then quantify the connection between energy balance and lapse rate regimes (Section~\ref{sec:diagnostics}). We use idealized climate models to formulate and test hypotheses that explain the seasonality of energy balance regime transitions in the Northern Hemisphere (Section~\ref{sec:hypo}).\added{ Furthermore, we show that energy balance regimes provides a useful guide to the vertical structure of the warming response latitudinally.} Lastly, the results are summarized and discussed (Section~\ref{sec:conclusion}).
...
\section{Energy balance regimes in reanalysis data} \label{sec:diagnostics}
    ...
    In the Southern high latitudes, \replaced{the seasonality of sensible heat flux (orange line, Fig.~\ref{fig:rea-r1-decomp-pole}d) contributes to the seasonality of $R_1$ that is comparable to the Northern high latitudes (compare Fig.~\ref{fig:rea-r1-decomp-pole}a and c).}{the dynamic and radiative components also largely cancel,} \replaced{However, there is no regime transition in the Southern high latitudes because the annual mean $R_1=1.36$ is farther from the regime transition threshold ($R_1=0.9$).}{but there is no regime transition.}\deleted{ The Southern high latitudes remain in RAE yearround consistent with a smaller summertime latent heat flux (2 Wm$^{-2}$, blue line in Fig.~\ref{fig:rea-r1-decomp-pole}d) and a larger annual mean $R_1$ ($\overline{R_1}=1.36$).} The larger annual mean $R_1$ is associated with smaller radiative cooling and a persistent downward sensible heat flux (gray and orange lines in Fig.~\ref{fig:rea-r1-decomp-pole}d).
...
\section{Testing hypotheses to explain seasonal regime transitions using idealized climate models} \label{sec:hypo}
  ...
  \end{equation}
  where we assumed that the radiative component is negligible\added{. This is satisfied for the Northern Hemisphere midlatitudes} (\replaced{see}{c.f.} Fig.~\ref{fig:rea-r1-decomp-mid}a)\added{, but not for the Southern Hemisphere midlatitudes (Fig.~\ref{fig:rea-r1-decomp-mid}c).} In order to close equation~(\ref{eq:mse-toasfc-approx}), which depends on the unknown surface temperature tendency, we make use of the EBM (see Section~\ref{sec:methods}\ref{subsec:models} and Appendix~C for more details). Following the EBM, we can write equation~(\ref{eq:mse-toasfc-approx}) as
  ...
  The dependence of\added{ the amplitude of} $\Delta R_1$ on mixed layer depth in AQUA is qualitatively consistent with the EBM prediction (compare stars to solid black line in Fig.~\ref{fig:amp-r1-echam}a). \deleted{While $\Delta R_1$ is less sensitive to the mixed layer depth in the EBM than in AQUA at very small mixed layer depths, it captures the seasonal amplitude of surface temperature (Fig.~\ref{fig:amp-r1-echam}b). }\added{Both AQUA and EBM predict that t}\deleted{T}he midlatitude regime transition\deleted{ from RCAE to RCE}\added{ only occurs for shallow mixed layer depths. This is consistent with the observed hemispheric asymmetry in the midlatitude regime transition, where the regime transition is absent in the ocean-dominated (deep mixed layer) Southern midlatitudes whereas the regime transition occurs in the Northern midlatitudes, where the land fraction is higher (shallower mixed layer depth). Specifically, the regime transition occurs} in the EBM\deleted{ occurs} for $d \le 16$ m and in AQUA\deleted{ occurs} for $d \le 20$ m (intersection of the line and stars with the orange region in Fig.~\ref{fig:amp-r1-echam}a).

  \added{The phase of $\Delta R_1$ is also a function of the mixed layer depth in AQUA (Fig.~\ref{fig:echam-rce-mld}). The seasonal minimum of $R_1$ occurs as early as June in AQUA with a 3 m mixed layer depth and as late as September with a 50 m mixed layer depth. While AQUA with a 3 m mixed layer depth captures the phase of $R_1$ in the reanalysis Northern midlatitudes, it overpredicts the amplitude by a factor of three (compare cyan line in Fig.~\ref{fig:echam-rce-mld} with black line in Fig.~\ref{fig:rea-r1-decomp-mid}a). This suggests that the seasonality in the Northern Hemisphere may be the result of zonal variations in surface heat capacity (land vs ocean). In the following, we focus on AQUA with 15 and 40 m mixed layer depths, which capture the reanalysis mean annual mean $R_1$ in the Northern and Southern Hemisphere midlatitudes, respectively.}
  ...
  In the Arctic, surface latent heat flux is strongly affected by the presence of sea ice, which \replaced{reduces}{increases} the albedo\added{, reduces the surface heat capacity,} and modulates heat exchange between the ocean and atmosphere (Andreas et al. 1989; Maykut 1982). We therefore expect sea ice to play a key role in the seasonality of energy balance regimes in the Northern high latitudes. We test the importance of sea ice using mechanism denial experiments where AQUA is configured with and without thermodynamic sea ice\added{ for various mixed layer depths} (Section~\ref{sec:methods}\ref{subsec:models}).

  \added{When AQUA is configured with sea ice, a high latitude regime transition occurs in the high latitudes (colored lines intersect blue region in Fig.~\ref{fig:echam-rae-mld}a). The boundary layer lapse rate deviation shows that an inversion exists when the high latitudes are in RAE but disappears when the high latitudes are in RCAE (colored lines overlapping the white region in Fig.~\ref{fig:echam-rae-mld}c). The duration of the RCAE and inversion-free regimes increase with increasing mixed layer depth. Whereas the timing of RCAE and inversion-free regimes exhibited a discrepancy in the reanalyses (Fig.~\ref{fig:rea-r1-ga-temporal}b), no such discrepancy occurs in AQUA with ice (compare Fig.~\ref{fig:echam-rae-mld}a and \ref{fig:echam-rae-mld}c). This suggests that a more comprehensive sea ice representation such as one that includes sea ice dynamics may be necessary to understand the discrepancy between energy balance and lapse rate regimes in the Northern high latitudes.}

  \added{When AQUA is configured without sea ice, there is no high latitude regime transition for mixed layer depths greater than 5 m (Fig.~\ref{fig:echam-rae-mld}b). Consistently, there is no surface inversion yearround for mixed layer depths above 5 m (Fig.~\ref{fig:echam-rae-mld}d). Thus, sea ice is required for a high latitude regime transition for mixed layer depths that are typically representative of an ocean surface.}
  
  \added{The existence of a high latitude regime transition in 5 m AQUA without sea ice indicates that a high surface albedo is not necessary for its existence. However, the RAE regime in this configuration occurs only during springtime and the annual mean $R_1$ is lower than that in the observed Northern high latitudes (compare green line in Fig.~\ref{fig:echam-rae-mld}b to black line in Fig.~\ref{fig:rea-r1-ga-temporal}b). Thus, the albedo effect of sea ice must also be considered to fully reproduce the observed seasonality of energy balance regimes.}
...
\added{Here, we quantify whether modern energy balance regimes allow us to better understand the regional temperature response to CO$_2$ forcing using CMIP5 simulations (RCP8.5 minus historical, Section~\ref{sec:methods}\ref{subsec:cmip}). The expectation that warming is amplified aloft in regions of RCE and amplified at the surface in regions of RAE \citep{payne2015} applies only if energy balance regimes remain unchanged. Furthermore, while the vertical temperature response in columns of RCE is expected to follow convective adjustment regardless of the type of forcing, the response in columns of RAE have been shown to be sensitive to the type of forcing \citep{cronin2016}. We hypothesize that the sensitivity of the lapse rate to forcing also applies for columns of RCAE, where the degree to which the RCAE lapse rate is subadiatic depends on the profile of advective heating. Thus, the response over regions of RAE and RCAE may be sensitive to the change in $R_1$ with warming.}

\added{In the annual mean, the CMIP5 multimodel mean latitudinal structure of energy balance regimes remains unchanged by the end of the century with the exception of the Arctic (compare solid and dashed lines in Fig.~\ref{fig:cmip5-r1-ann-rcp}). In the Arctic, the RAE regime contracts poleward from $75^\circ$N to $80^\circ$N. This is associated with a decrease in $R_1$ in the Arctic on the order of 0.1. In the Southern midlatitudes, $R_1$ increases by a similar magnitude but does not lead to a regime change.}

\added{The annual mean vertical temperature response to global warming is a clear function of the climatological energy balance regime, i.e. the value of $R_1$ (Fig.~\ref{fig:cmip5-dtemp-binned-r1}). In particular, the vertical temperature response to warming in modern RCE exhibits amplified warming aloft and in modern RAE regions exhibits amplified warming at the surface. The temperature response is a non-monotonic function of modern $R_1$. Specifically, warming over the Arctic (represented by the blue lines in Fig.~\ref{fig:cmip5-dtemp-binned-r1}) is double that over the Antarctic (purple lines in Fig.~\ref{fig:cmip5-dtemp-binned-r1}) despite the largest climatological values of $R_1$ occurring over Antarctica. The hemispheric asymmetry in polar warming is consistent with the change in $R_1$ with warming: whereas $R_1$ remains unchanged over the Antarctic, $R_1$ decreases in the Arctic, moving it closer to the inversion-free RCAE regime (Fig.~\ref{fig:cmip5-r1-ann-rcp}). Suppressed warming over Antarctica is thought to be associated with the presence of Antarctic topography \citep{salzmann2017,hahn2020,singh2020} and the long transient response of the Southern Ocean overturning circulation \citep{armour2016}.}

\added{Next, we examine the seasonality of the CMIP5 multimodel mean change in $R_1$ (Fig.~\ref{fig:cmip5-dr1}a). We focus on changes in $R_1$ in the mid and high latitudes as we hypothesized that changes in $R_1$ only influence the temperature response in regions of RCAE and RAE. The $R_1$ response in the Northern Hemisphere exhibits strong seasonality. In the Northern high latitudes, $R_1$ decreases (moving toward RCAE) from September through April and increases (moving toward RAE) from May through August. The Northern midlatitude $R_1$ response is opposite that of the Northern high latitudes, with a decrease (moving toward RCE) during summertime and increase (moving toward RAE) during wintertime. The $R_1$ response in the Southern Hemisphere exhibits weaker seasonality, with an yearround increase in the midlatitudes and decrease in the high latitudes.}

\added{Changes in $R_1$ can be used to infer their influence on the vertical temperature response. Regions of decreasing (increasing) $R_1$ is associated with an increase (decrease) in MSE flux divergence (compare Fig.~\ref{fig:cmip5-dr1}a and \ref{fig:cmip5-dr1}b) and an increase (decrease) in surface turbulent fluxes (compare Fig.~\ref{fig:cmip5-dr1}a and \ref{fig:cmip5-dr1}c). As the forcing warms (cools) the surface and cools (warms) the free troposphere in regions of decreasing (increasing) $R_1$, we expect regions of decreasing $R_1$ (red contours in Fig.~\ref{fig:cmip5-dr1}a) to be associated with enhanced surface warming, and vice versa.}

\added{The seasonality of the vertical temperature response (expressed as the ratio of the temperature response at $\sigma=0.3$ to $\sigma=1.0$, see Fig.~\ref{fig:cmip5-dtemp}) is consistent with the seasonality of modern $R_1$ (Fig.~\ref{fig:cmip5-r1-ann-rcp}a) and its response to warming \ref{fig:cmip5-dr1}a). In regions of modern RCE such as the tropics yearround and the Northern midlatitude summertime (equatorward of the thick orange line in Fig.~\ref{fig:cmip5-dr1}a), warming is amplified in the upper troposphere (blue contours in Fig.~\ref{fig:cmip5-dtemp}). In regions of modern RCAE and RAE, we must also consider the change in $R_1$ to understand the vertical temperature response. Where $R_1$ decreases such as the Antarctic yearround and the Arctic outside summer (red regions in Fig.~\ref{fig:cmip5-dr1}a), warming is amplified at the surface (red regions in Fig.~\ref{fig:cmip5-dtemp}). Where $R_1$ increases such as the Northern high latitudes during summertime and the Southern midlatitudes yearround (blue regions in Fig.~\ref{fig:cmip5-dr1}a), warming is amplified in the upper troposphere (blue regions in Fig.~\ref{fig:cmip5-dtemp}).}

% \added{Amplified upper tropospheric warming in regions of modern RCAE (Northern high latitudes and Southern midlatitudes) is associated with regionally suppressed surface warming (Fig.~\ref{fig:cmip5-dtemp-dr1}c). This is in contrast to amplified upper tropospheric warming in regions of modern RCE (tropics and Northern midlatitudes), which is associated with regionally enhanced upper tropospheric warming (Fig.~\ref{fig:cmip5-dtemp-dr1}b). This suggests that amplified upper tropospheric warming in regions of modern RCAE arises not due to moist adiabatic adjustment, but rather the slow transient response of the ocean overturning circulation that delays surface warming. The ocean heat uptake that is associated with delayed warming of the ocean surface is reflected in the change in $R_1$. In particular, $\Delta R_1$ increases in the Northern high latitudes during summertime and the Southern midlatitudes yearround (blue contours in Fig.~\ref{fig:cmip5-dtemp-dr1}d) consistent with enhanced downward sensible heat flux and an associated increase in MSE flux convergence into these regions.}

% \added{In the Northern Hemisphere midlatitudes, the vertical temperature response is amplified in the upper troposphere from May through October, surface amplified from December through March, and mixed during April and November (Fig.~\ref{fig:cmip5-dtemp-mid}a,b). The presence of upper troposphere amplified warming from May through July is consistent with the RCE regime in the base-state climate (Fig.~\ref{fig:cmip5hist-r1-decomp-mid}a). However, upper troposphere amplified warming is not unique to regions of base-state RCE (August through October is RCAE). This is further illustrated in the Southern Hemisphere midlatitudes where the vertical temperature response is amplified in the upper troposphere (Fig.~\ref{fig:cmip5-dtemp-mid}c,d) despite being in the RCAE regime yearround in the base climate (Fig.~\ref{fig:cmip5hist-r1-decomp-mid}c).}

% \added{In the Northern Hemisphere high latitudes, the vertical temperature response is surface amplified from September through April (Fig.~\ref{fig:cmip5-dtemp-polar}a,b). This is exactly consistent with the seasonality of the base-state inversion (blue line in Fig.~\ref{fig:cmip5hist-r1-ga-temporal}b) and consistent with the base-state RAE regime with the exception of July and August (solid black line in Fig.~\ref{fig:cmip5hist-r1-ga-temporal}b).   comparison, the vertical temperature response in the Southern Hemisphere high latitudes is surface amplified yearround (Fig.~\ref{fig:cmip5-dtemp-polar}c,d), consistent with the base-state RAE regime persisting yearround (Fig.~\ref{fig:cmip5hist-r1-decomp-pole}c).}

\section{Summary and Discussion}\label{sec:conclusion}
\subsection{Summary}
We quantified \replaced{where and when}{when and where} energy balance regimes are observed and their connection to lapse rates for Earth's modern climate\added{ and the vertical temperature response to increased CO$_2$}. We used the vertically-integrated MSE budget to define a nondimensional number $R_1=\frac{\partial_t m + \partial_y (vm)}{R_a}$ that quantifies regions of RCE ($R_1\le0.1$), RAE ($R_1\ge0.9$), and RCAE ($0.1<R_1<0.9$). In the annual mean, the RCE regime occurs equatorward of $40^\circ$, where the free tropospheric lapse rate deviates less than 13\% from a moist adiabat. The RAE regime occurs poleward of $80^\circ$N and $70^\circ$S consistent with where surface inversions occur. Lastly, the RCAE regime occurs between $40$--$70^\circ$S and $40$--$80^\circ$N, where the free tropospheric lapse rate deviates 13\% to 35\% from a moist adiabat.
...
\added{Climatological energy balance regimes are broadly consistent with expected vertical temperature responses over RCE and RAE. We demonstrate using state-of-the-art GCMs that regions of RCE in the base-state climate exhibit upper tropospheric amplified warming, RAE surface amplified warming, and RCAE a mix of both features.}
...
\added{The connection between base-state energy balance regimes and the vertical temperature response demonstrated here using state-of-the-art GCMs complements the findings of \cite{payne2015}, who demonstrated that the vertical temperature response in the low and high latitudes are consistent with predictions of idealized column models of RCE and RAE, respectively. While columns of RCE and RAE are expected to follow upper troposphere and surface amplified warming, respectively, the response over RCAE is not as well understood. \cite{frierson2006a} and \cite{frierson2008} suggest that moist convection plays an important role in controlling the midlatitude temperature response over an ocean-dominated surface such as the Southern Hemisphere midlatitudes, but further work is required to understand the response in the presence of land.}
...
\begin{figure}
  \centering
  \noindent\includegraphics[width=\textwidth]{/project2/tas1/miyawaki/projects/002/figures/gcm/mmm/historical/198001-200512/1.00/energy-flux/lo/ann/mse_old-r1z-rcp.png}
...
\begin{figure}
  \centering
  \noindent\includegraphics[width=\textwidth]{/project2/tas1/miyawaki/projects/002/figures/gcm/mmm/historical/198001-200512/1.00/dtempr/lo/dtempr_mon_lat.png}
...
% \begin{figure}
%   \centering
%   \noindent\includegraphics[width=\textwidth]{/project2/tas1/miyawaki/projects/002/figures_post/final/dtempsi_mid/dtempsi_mid_cmip5hist.pdf}\\
...
% \begin{figure}
%   \centering
%   \noindent\includegraphics[width=\textwidth]{/project2/tas1/miyawaki/projects/002/figures_post/final/dtempsi_polar/dtempsi_polar_cmip5hist.pdf}\\
