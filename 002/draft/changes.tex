...
Quantifying the latitudinal dependence of observed energy balance regimes would allow us to assess where idealized models that assume RCE or RAE hold.\added{ This helps synthesize results across the climate model hierarchy by making more explicit the regional extent to which insights gained from single-column and limited-domain RCE and RAE models are valid.} This is particularly important for RCE, which has become a standard idealized configuration for tropical theories \citep[e.g.,][]{emanuel1996,nilsson1999,romps2014,singh2015} and simulations \citep[][and the references therein]{wing2018}.\added{ For example, studies on the projected increase in tropical precipitation are commonly based on cloud resolving models configured under the assumption of RCE \citep{romps2011, pendergrass2016, abbott2020}.} Recent work by \cite{jakob2019} quantified regions of RCE using the dry static energy (DSE) budget and a dimensional threshold (sum of radiative cooling, latent heating associated with precipitation, and surface sensible heating $< \pm 50$ Wm$^{-2}$). They find that RCE is approximately satisfied over large spatial ($>5000$ km) and temporal ($>$ daily) scales in the tropics. However, the latitudinal distribution of RCE outside the tropics and the distribution of RAE and RCAE have not been investigated.
  
Quantifying the link between energy balance and lapse rate regimes would also allow us to better understand the regional response to CO$_2$ forcing. For example, regions of RCE that exhibit a moist adiabatic lapse rate will have amplified warming aloft in response to increased CO$_2$ \citep{held1993, romps2011}. In contrast, regions of RAE that exhibit a surface inversion will have amplified warming at the surface in response to increased CO$_2$ \citep{held1993, cronin2016}.\added{ Thus, the geographic and seasonal structure of energy balance regimes in a mean state climate can be used to make predictions of the vertical structure of warming. While \cite{payne2015} demonstrate that idealized column models of RCE and RAE predict amplified warming in the upper troposphere and the surface, respectively, the link between energy balance regimes and vertical temperature responses has not yet been investigated in more comprehensive GCMs.}

We therefore seek to answer the questions: when and where do energy balance regimes (RCE, RAE, and RCAE) occur on modern Earth, and how closely are they linked to lapse rate regimes (moist adiabatic and surface inversion)\added{ and the vertical temperature response to CO$_2$ forcing (upper troposphere and surface amplified)}? To answer these questions, we develop a quantitative definition for energy balance regimes using the nondimensionalized MSE budget (Section~\ref{sec:methods}\ref{subsec:mse}). We use this definition to quantify where and when energy balance regimes occur in the annual mean and seasonally on modern Earth using reanalysis and CMIP5 data. We then quantify the connection between energy balance and lapse rate regimes (Section~\ref{sec:diagnostics}). We use idealized climate models to formulate and test hypotheses that explain the seasonality of energy balance regime transitions in the Northern Hemisphere (Section~\ref{sec:hypo}).\added{ Furthermore, we evaluate the consistency between base-state energy balance regimes and vertical temperature responses to increased CO$_2$.} Lastly, the results are summarized and discussed (Section~\ref{sec:conclusion}).
...
\section{Methods}\label{sec:methods}
    ...
    At the simple end, we use the\added{ analytical} EBM of \cite{rose2017}. The EBM is an equation for the zonal mean surface temperature:
    \begin{equation}
...
\section{Energy balance regimes in reanalysis data} \label{sec:diagnostics}
    ...
    \replaced{In the Northern high latitudes, the dynamic and radiative components largely compensate; however, the extent to which they do not exactly compensate leads to a small seasonality in $R_1$ and the RAE to RCAE regime transition.}{The RAE to RCAE regime transition in the Northern high latitudes is a small residual of the dynamic and radiative components} (Fig.~\ref{fig:rea-r1-decomp-pole}a).\added{ Since $\overline{R_1}\approx 1$ in the Northern high latitudes (see black line in Fig.~\ref{fig:rea-r1-ann}), Equation~\ref{eq:r1-dev} simplifies to:}
    \begin{align}
...
\section{Testing hypotheses to explain seasonal regime transitions using idealized climate models} \label{sec:hypo}
  ...
  \end{equation}
  where we assumed that the radiative component is negligible\added{. This is well satisfied for the Northern Hemisphere midlatitudes} (c.f. Fig.~\ref{fig:rea-r1-decomp-mid}a)\added{, but may lead to errors in the phase of $\Delta R_1$ for deeper mixed layers, such as in the Southern Hemisphere midlatitudes (Fig.~\ref{fig:rea-r1-decomp-mid}c).} In order to close equation~(\ref{eq:mse-toasfc-approx}), which depends on the unknown surface temperature tendency, we make use of the EBM (see Section~\ref{sec:methods}\ref{subsec:models} and Appendix~C for more details). Following the EBM, we can write equation~(\ref{eq:mse-toasfc-approx}) as
  ...
  Without Antarctic topography, $\overline{R_1}$ decreases from 1.30 to 1.12 in the Southern high latitudes (compare Fig.~\ref{fig:hahn-aa}a to \ref{fig:hahn-aa}b).\added{ Decomposing the difference in $R_1$ between the simulation with and without Antarctic topography following Equation~(5) shows that there is a large compensation between the radiative and dynamic components (Fig.~S1). This reflects the two competing effects of removing topography on $R_1$: 1) radiative cooling increases (a strong negative quantity) due to the optically thicker atmosphere (gray line in Fig.~S2), which acts to decrease $R_1$ (gray line in Fig.~S1), and 2) MSE flux convergence increases (red line in Fig.~S2), which acts to increase $R_1$ (red line in Fig.~S1). The radiative effect dominates over the compensating dynamic effect with the exception of December and January.} \replaced{Since}{However, because} topography does not significantly affect summertime $R_1$, the Southern high latitudes without Antarctic topography continue to remain in RAE yearround (dashed line remains in the blue region in Fig.~\ref{fig:hahn-aa}b). In both configurations of CESM, Antarctic latent heat flux remains small (below 5 Wm$^{-2}$, compare dashed and solid lines in Fig.~\ref{fig:hahn-aa}c,d). Thus, while Antarctic topography partially explains the large hemispheric asymmetry in annual mean $R_1$, it does not significantly affect the seasonality of energy balance regimes in the Antarctic.

\section{Climatological energy balance regimes and vertical temperature responses to climate change} \label{sec:vtemp}
\added{A promising application of quantifying the seasonal and latitudinal dependence of energy balance regimes is predicting key features of the vertical temperature response to CO$_2$ forcing. We find that there is a clear relationship between the climatological $R_1$ and the vertical temperature response: columns in RCE exhibit amplified warming in the upper troposphere (yellow/orange lines in Fig.~\ref{fig:cmip5-dtemp-binned-r1}) and RAE amplified warming near the surface (blue/purple lines in Fig.~\ref{fig:cmip5-dtemp-binned-r1}). Columns in RCAE include a mix of both amplified warming in the upper troposphere or surface, depending on the value of $R_1$ (light blue/green lines in Fig.~\ref{fig:cmip5-dtemp-binned-r1}).}
...
\subsection{Summary}
We quantified when and where energy balance regimes are observed and their connection to lapse rates for Earth's modern climate\added{ and the vertical temperature response to increased CO$_2$}. We used the vertically-integrated MSE budget to define a nondimensional number $R_1=\frac{\partial_t m + \partial_y (vm)}{R_a}$ that quantifies regions of RCE ($R_1\le0.1$), RAE ($R_1\ge0.9$), and RCAE ($0.1<R_1<0.9$). In the annual mean, the RCE regime occurs equatorward of $40^\circ$, where the free tropospheric lapse rate deviates less than 13\% from a moist adiabat. The RAE regime occurs poleward of $80^\circ$N and $70^\circ$S consistent with where surface inversions occur. Lastly, the RCAE regime occurs between $40$--$70^\circ$S and $40$--$80^\circ$N, where the free tropospheric lapse rate deviates 13\% to 35\% from a moist adiabat.
