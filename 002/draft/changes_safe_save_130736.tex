...
\title{\replaced{Where and when}{When and where} do Radiative Convective and Radiative Advective Equilibrium regimes occur on modern Earth?}
...
\abstract{Energy balance and lapse rate regimes qualitatively characterize the low, mid, and high latitudes of Earth's modern climate. Currently we do not have a complete quantitative understanding of the spatio-temporal structure of energy balance regimes (e.g., Radiative Convective Equilibrium\replaced{,}{ or} RCE, and Radiative Advective Equilibrium\replaced{,}{ or} RAE) and their connection to lapse rate regimes\added{ (moist adiabat and surface inversion)}. Here we use the vertically-integrated moist static energy budget to define a nondimensional number that quantifies \replaced{where and when}{when and where} RCE and RAE are approximately satisfied in Earth's modern climate. We find RCE exists yearround in the tropics and in the Northern midlatitudes during summertime. RAE exists yearround over Antarctica and in the Arctic with the exception of early summer. We show that the lapse rates in RCE and RAE\deleted{ regimes in reanalyses and CMIP5 models} are\deleted{ broadly} consistent with moist adiabatic and surface inversion lapse rates, respectively. We use idealized models (energy balance and aquaplanet\deleted{ models}) to test the following hypotheses: 1)\deleted{ the} RCE\deleted{ regime} occurs during midlatitude summer for land-like (small heat capacity) surface conditions and 2) sea ice is necessary for the existence of\deleted{ the} RAE\deleted{ regime} over a polar ocean, such as the Arctic. Consistent with \replaced{1)}{the first hypothesis}, an aquaplanet\deleted{ model} configured with a shallow mixed layer depth transitions to RCE in the midlatitudes during summertime whereas it does not for a deep mixed layer depth. Furthermore, we confirm \replaced{2)}{the second hypothesis} using mechanism-denial aquaplanet experiments with and without thermodynamic sea ice.\added{ Finally, we show that modern energy balance regimes provide a useful guide to the vertical structure of the global warming response latitudinally and seasonally.}}
...
Quantifying the latitudinal\added{ and seasonal} dependence of observed energy balance regimes would allow us to assess where\added{ and when} idealized models that assume RCE or RAE hold.\added{ This would be useful for several reasons.}\deleted{ This is particularly important for RCE, which has become a standard idealized configuration for tropical theories (e.g., Emanuel and Bister 1996; Nilsson and Emanuel 1999; Romps 2014; Singh and O'Gorman 2015) and simulations (Wing et al. 2018, and the references therein).}\added{ First, the results of many studies investigating tropical climate and its response to warming assume RCE. For example our understanding of the tropical precipitation, cloud, and circulation responses to warming are rooted on models that that assume RCE} \citep[e.g.,][]{romps2011,popke2013,pendergrass2016,merlis2019}\added{. Quantifying the latitudinal and seasonal dependence of RCE can guide where and when these insights are applicable in Earth's modern climate.} \cite{jakob2019} \added{took a first step in this direction by quantifying the spatial and temporal scales that RCE is satisfied in the tropics. However, they did not examine the possible occurrence of RCE outside of the tropics or energy balance regimes more generally (RAE and RCAE).} 

\added{Second, knowledge of energy balance regimes on modern Earth could provide a useful guide for interpreting the vertical structure of the warming response geographically and seasonally. For regions of modern Earth that are in RCE and remain in RCE in the future, the expectation is that the warming response maximizes aloft}\deleted{Quantifying the link between energy balance and lapse rate regimes would also allow us to better understand the regional response to CO$_2$ forcing. For example, regions of RCE that exhibit a moist adiabatic lapse rate will have amplified warming aloft in response to increased CO$_2$} \citep{held1993, romps2011}. \replaced{For regions of modern Earth that are in RAE and remain in RAE in the future, the expectation is that the warming response maximizes at the surface (assuming that the radiative forcing dominates over the surface forcing or advective forcing)}{In contrast, regions of RAE that exhibit a surface inversion will have amplified warming at the surface in response to increased CO$_2$} \citep{held1993, cronin2016}.\added{Ultimately, it is an open question whether energy balance regimes defined in the modern climate provide a useful guide to the vertical structure of warming latitudinally and seasonally.}
...
\replaced{Here, we seek to answer the questions}{We therefore seek to answer the questions}: \replaced{where and when}{when and where} do energy balance regimes (RCE, RAE\deleted{, and RCAE}) occur on modern Earth, and how closely are they linked to lapse rate regimes (moist adiabatic and surface inversion)? To answer these questions, we develop a \replaced{nondimensional number based on the MSE budget to quantify}{quantitative definition for} energy balance regimes\deleted{ using the nondimensionalized MSE budget} (Section~\ref{sec:methods}\ref{subsec:mse}). We use this \replaced{nondimensional number to quantify}{definition to quantify} where and when energy balance regimes occur \replaced{latitudinally}{in the annual mean} and seasonally on modern Earth using reanalysis and CMIP5 data. We then\added{ also} quantify the connection between energy balance and lapse rate regimes (Section~\ref{sec:diagnostics})\replaced{ and}{. We} use idealized climate models to formulate and test hypotheses that explain the seasonality of energy balance regime transitions in the Northern Hemisphere (Section~\ref{sec:hypo}).\added{ Finally, we show that energy balance regimes provides a useful guide to the vertical structure of the warming response latitudinally (Section~\ref{sec:vtemp}).} Lastly, the results are summarized and discussed (Section~\ref{sec:conclusion}).
...
\section{Methods}\label{sec:methods}
    ...
    \end{equation}
    where $\langle[\partial_t m]\rangle$ represents atmospheric MSE storage. In order to nondimensionalize equation~(\ref{eq:mse}), we divide by radiative cooling $R_a$, which is sign definite \replaced{for Earth's modern climate}{in the zonal mean}:
    \begin{equation}
...
\section{Energy balance regimes in reanalysis data} \label{sec:diagnostics}
    ...
    The RAE regime, defined by $R_1 \ge 0.9$, occurs poleward of $\approx 80^\circ$N and $\approx 70^\circ$S in the annual mean (black line overlapping blue region in Fig.~\ref{fig:rea-r1-ann}b). The reanalysis spread in the high latitudes is large in both hemispheres due to high uncertainty in the estimation of surface turbulent fluxes \citep{tastula2013,graham2019}. The largest values of $R_1$ are found over Antarctica whereas $R_1$ is close to the RCAE threshold in the Arctic. Consistently,\added{ the latitudinal extent of the RAE regime coincides with that of the surface inversion lapse rate regime. In paricular,} the region where $R_1\ge 0.9$ exhibits $>100\%$ deviation from a moist adiabat indicating a surface inversion (blue line in Fig.~\ref{fig:rea-r1-ann}b).

    Lastly, the RCAE regime, defined by $0.1 < R_1 < 0.9$, occurs between $40$--$80^\circ$N and $40$--$70^\circ$S in the annual mean (black line overlapping the white region in Fig.~\ref{fig:rea-r1-ann}a).\added{ The RCAE regime coincides with the mixed lapse rate regime, where the lapse rate is more stable than a moist adiabat and does not exhibit a surface inversion. In particular,} \replaced{t}{T}he free tropospheric lapse rate in the region where $0.1 < R_1 < 0.9$ deviates $+13$\% to $+35$\% from a moist adiabat.

    \subsection{Seasonality of energy balance regimes} \label{subsec:seasonality}
    % The seasonality of $R_1$ is weak in the Southern Hemisphere, such that the latitudinal extent of RCE (equatorward of $40^\circ$S), RAE (poleward of $70^\circ$S), and RCAE regimes (between $40$--$70^\circ$S) is largely the same throughout the year (Fig.~\ref{fig:rea-r1-dev}a). Consistently, both the seasonality of free tropospheric lapse rate deviation\added{ in the midlatitudes} (Fig.~\ref{fig:rea-r1-dev}b) and the boundary layer lapse rate deviation in the high latitudes (Fig.~\ref{fig:rea-r1-dev}c) \replaced{are weak}{exhibit weak seasonality} in the Southern Hemisphere.
    The seasonality of $R_1$ is weak in the Southern Hemisphere, such that the latitudinal extent of RCE (equatorward of $40^\circ$S), RAE (poleward of $70^\circ$S), and RCAE regimes (between $40$--$70^\circ$S) is largely the same throughout the year (Fig.~\ref{fig:rea-r1-dev}a). Consistently, \replaced{the midlatitudes exhibit a mixed lapse rate regime and the high latitudes exhibit a surface inversion yearround (Fig.~\ref{fig:rea-r1-dev}b,c)}{both the seasonality of free tropospheric lapse rate deviation (Fig.~\ref{fig:rea-r1-dev}b) and the boundary layer lapse rate deviation in the high latitudes (Fig.~\ref{fig:rea-r1-dev}c) exhibit weak seasonality in the Southern Hemisphere}.
    ...
    \end{align}
    \replaced{where we have used the MSE budget (equation~\ref{eq:mse}). Thus, the regime transition in the Northern high latitudes is connected to the seasonality of surface turbulent fluxes, which is associated with}{The small residual is associated with} an increase in latent heat flux of up to 11 Wm$^{-2}$ \replaced{from}{during} May \replaced{to September}{and June} (blue line, Fig.~\ref{fig:rea-r1-decomp-pole}b). 
    
    In the Southern high latitudes, \replaced{the seasonality of surface turbulent fluxes and thus $R_1$ is comparable to that of the Northern high latitudes (compare Fig.~\ref{fig:rea-r1-decomp-pole}a,b and \ref{fig:rea-r1-decomp-pole}c,d).}{the dynamic and radiative components also largely cancel,} \replaced{However, there is no regime transition in the Southern high latitudes because the annual mean $R_1=1.36$ is farther from the regime transition threshold for RAE ($R_1=0.9$).}{but there is no regime transition.}\deleted{ The Southern high latitudes remain in RAE yearround consistent with a smaller summertime latent heat flux (2 Wm$^{-2}$, blue line in Fig.~\ref{fig:rea-r1-decomp-pole}d) and a larger annual mean $R_1$ ($\overline{R_1}=1.36$).} The larger annual mean $R_1$ is associated with smaller radiative cooling and a persistent downward sensible heat flux (gray and orange lines in Fig.~\ref{fig:rea-r1-decomp-pole}d).
...
\section{Testing hypotheses to explain seasonal\added{ energy balance} regime transitions\deleted{ using idealized climate models}} \label{sec:hypo}
  ...
  \end{equation}
  where we assumed that the radiative component is negligible\added{ as seen in the Northern Hemisphere where the regime transition occurs} (\replaced{see}{c.f.} Fig.~\ref{fig:rea-r1-decomp-mid}a). In order to close equation~(\ref{eq:mse-toasfc-approx}), which depends on the unknown surface temperature tendency, we make use of the EBM (see Section~\ref{sec:methods}\ref{subsec:models} and Appendix~C for more details). Following the EBM, we can write equation~(\ref{eq:mse-toasfc-approx}) as
  ...
  \replaced{Sea ice in the Arctic affects the surface albedo, surface heat capacity, and surface turbulent fluxes}{In the Arctic, surface latent heat flux is strongly affected by the presence of sea ice, which increases the albedo and modulates heat exchange between the ocean and atmosphere} \citep{andreas1979, maykut1982}. We therefore expect sea ice to play a key role in the seasonality of energy balance regimes in the Northern high latitudes. We test the importance of sea ice using mechanism denial experiments where AQUA is configured with and without thermodynamic sea ice\added{ for various mixed layer depths} (Section~\ref{sec:methods}\ref{subsec:models}).


  \added{We investigate the albedo effect of sea ice by varying the mixed layer depth in AQUA with ice. This leads to a large range of seasonalities in surface albedo, sea ice fraction, and sea ice depth (Fig.~\ref{fig:echam-ice-props}). A high latitude regime transition occurs for all mixed layer depths (Fig.~\ref{fig:echam-rae-mld}a). This suggests that the surface albedo does not control the existence of a high latitude regime transition within the range of surface albedo seasonalities obtained in these experiments. However, the duration of the RCAE regime is sensitive to the mixed layer depth.}
  
  \added{The RAE regime transition coincides with the transition to a surface inversion lapse rate regime (colored lines overlapping the white region in Fig.~\ref{fig:echam-rae-mld}b). Whereas the timing of RCAE and inversion-free regimes exhibited a discrepancy in the reanalyses (Fig.~\ref{fig:rea-r1-ga-temporal}b), no such discrepancy occurs in AQUA with ice (compare Fig.~\ref{fig:echam-rae-mld}a and \ref{fig:echam-rae-mld}b). This suggests that physical mechanisms that are missing in AQUA, such as zonal asymmetry in sea ice properties or sea ice dynamics, may be necessary to understand the discrepancy between energy balance and lapse rate regimes in the Northern high latitudes.}

  \added{We isolate the surface heat capacity effect of sea ice by varying the mixed layer depth in AQUA without ice. In the absence of ice, the surface albedo is a constant value corresponding to the ocean albedo (0.07). A high latitude energy balance and lapse rate regime transitions occur for a shallow mixed layer depth (5 m, Fig.~\ref{fig:echam-rae-mld}c,d), confirming that surface heat capacity of sea ice plays a more important role on the existence of a high latitude regime transition. However, the RAE regime transition occurs during the spring equinox and is not consistent with that seen in reanalyses data. Thus, the AQUA simulations suggest that sea ice is required to capture a regime transition that is more representative of that seen in reanalysis data. Ultimately, the role of sea ice on $R_1$ and the high latitude regime transition is a combination of both the surface heat capacity (for its existence) and surface albedo effects (for its timing).}

  In the Antarctic, the annual mean value of $R_1$ is larger than the Arctic and there is no regime transition from RAE to RCAE. In the presence of Antarctic topography, the atmosphere is optically thinner, so that we expect atmospheric radiative cooling to be weaker\added{ \citep{jeevanjee2018}}, and thus $R_1=\frac{\partial_t m + \partial_y (vm)}{R_a}$ to be larger. We therefore investigate the impact of Antarctic topography on the seasonality of energy balance regimes using mechanism denial experiments. In particular, we use the CESM simulations configured with and without (flattened) Antarctic topography by \cite{hahn2020}.

  The control CESM simulation with Antarctic topography captures the seasonality of $R_1$ in the reanalyses and CMIP5 historical runs. In particular, the Northern high latitudes undergo a RAE to RCAE regime transition in June (solid line intersects the white region in Fig.~\ref{fig:hahn-aa}a) while the Southern high latitudes remain in RAE yearround (dashed line remains in the blue region in Fig.~\ref{fig:hahn-aa}a). Without Antarctic topography, $\overline{R_1}$ decreases from 1.30 to 1.12 in the Southern high latitudes (compare Fig.~\ref{fig:hahn-aa}a to \ref{fig:hahn-aa}b). However, because topography does not significantly affect summertime $R_1$, the Southern high latitudes without Antarctic topography continue to remain in RAE yearround (dashed line remains in the blue region in Fig.~\ref{fig:hahn-aa}b).\deleted{ In both configurations of CESM, Antarctic latent heat flux remains small (below 5 Wm$^{-2}$, compare dashed and solid lines in Fig.~\ref{fig:hahn-aa}c,d).} Thus, while Antarctic topography partially explains the large hemispheric asymmetry in annual mean $R_1$, it does not significantly affect the seasonality of energy balance regimes in the Antarctic.\added{ The result that neither the presence of topography nor the seasonality of albedo (the 25 m AQUA with ice has a constant high albedo similar to that over the Antarctic) alone can explain the lack of a regime transition suggests that multiple mechanisms may need to be considered together to understand the Antarctic $R_1$ seasonality.}

\section{Connecting modern energy balance regimes to the vertical structure of the temperature response to\added{ global} warming} \label{sec:vtemp}

\added{Here, we quantify whether modern energy balance regimes provide a useful guide to the vertical, latitudinal, and seasonal structure of the response to global warming. In particular, we expect columns currently in RCE to exhibit upper tropospheric amplified warming as long as it remains in RCE under climate change. On the other hand, we expect columns currently in RAE to exhibit surface amplified warming as long as it remains in RAE under climate change and the change in $R_1$ is sufficiently small.}

\added{In the annual mean, the CMIP5 multimodel mean latitudinal structure of energy balance regimes remain unchanged by the end of the century with the exception of the Arctic (compare solid and dashed lines in Fig.~\ref{fig:cmip5-r1-ann-rcp}a). Correspondingly, the vertical temperature response to global warming in modern RCE exhibits amplified warming aloft (red lines, $R_1<0.1$ in Fig.~\ref{fig:cmip5-r1-ann-rcp}b) and in modern RAE regions exhibits amplified warming at the surface (blue lines, $R_1>0.9$ in Fig.~\ref{fig:cmip5-r1-ann-rcp}b). Thus in the annual mean, $R_1$ is useful for interpreting the vertical temperature response to warming.} 

% \added{The temperature response is a non-monotonic function of modern $R_1$ in regions of modern RAE. Specifically, warming over the Arctic (represented by the light blue lines in Fig.~\ref{fig:cmip5-r1-ann-rcp}b) is double that over the Antarctic (dark blue lines in Fig.~\ref{fig:cmip5-r1-ann-rcp}b). The hemispheric asymmetry in polar warming shows that the temperature response in RAE is not only a function of modern $R_1$ but also the change in $R_1$ with warming: whereas $R_1$ remains unchanged over the Antarctic, $R_1$ decreases in the Arctic.}. 

\added{In the Southern Hemisphere, the seasonality of energy balance regimes remain largely unchanged by the end of the century (compare dashed and solid thick contours in Fig.~\ref{fig:cmip5-dtemp}). Additionally, the change in $R_1$ with global warming is small ($\le5\%$) over Antarctica (hatched region in Fig.~\ref{fig:cmip5-dtemp}). Consistently, regions of RCE exhibit amplified upper tropospheric warming (red filled contours in Fig.~\ref{fig:cmip5-dtemp}) and regions of RAE exhibit amplified surface warming (blue filled contours in Fig.~\ref{fig:cmip5-dtemp}) through the seasonal cycle in the Southern Hemisphere.}

\added{In the Northern Hemisphere, the seasonality of RCE remains unchanged. Consistently, the warming response is amplified in the upper troposphere in the Northern Hemisphere tropics yearround. However in the midlatitudes, there is a phase lag in the seasonality of the warming response. Whereas Northern midlatitudes are in the RCE regime from April to July, amplified upper tropospheric warming occurs from June to September (compare dashed/solid orange contour and the filled red contours in Fig.~\ref{fig:cmip5-dtemp}). This discrepancy matches that of the modern seasonality of energy balance and lapse rate regimes (Fig.~\ref{fig:rea-r1-ga-temporal}a).}

\added{The seasonality of energy balance regimes in the Arctic undergo major changes, including a shorter summertime RCAE regime transition and a new RCAE regime transition from October through November. Additionally, there are limited regions in the Arctic where changes in $R_1$ are small ($>5\%$, see hatched regions in Fig.~\ref{fig:cmip5-dtemp}). As a result, the applicability of modern energy balance regimes in interpreting the vertical structure of warming is limited in the Arctic. Understanding the connection between energy balance regimes and the warming response in the Northern Hemisphere thus requires further analysis on the role of atmospheric heat storage and changes in $R_1$, which will be explored in future work.}

\section{Summary and Discussion}\label{sec:conclusion}
\subsection{Summary}
We quantified \replaced{where and when}{when and where} energy balance regimes are observed\added{ in Earth's modern climate} and their connection to lapse rate\deleted{s} \added{regimes}\deleted{ for Earth's modern climate}.\added{ Additionally, we investigated whether modern energy balance regimes are a useful guide for interpreting the vertical temperature response to global warming.} We used the vertically-integrated \replaced{moist static energy}{MSE} budget to define a nondimensional number $R_1=\frac{\partial_t m + \partial_y (vm)}{R_a}$ that quantifies regions of RCE ($R_1\le0.1$), RAE ($R_1\ge0.9$), and RCAE ($0.1<R_1<0.9$). In the annual mean, the RCE regime occurs equatorward of $40^\circ$, \replaced{consistent with the occurence of the moist adiabatic lapse rate regime}{where the free tropospheric lapse rate deviates less than 13\% from a moist adiabat}. The RAE regime occurs poleward of $80^\circ$N and $70^\circ$S consistent with \replaced{the occurence of the}{where} surface inversion\replaced{ lapse rate regime}{s occur}. Lastly, the RCAE regime occurs between $40$--$70^\circ$S and $40$--$80^\circ$N, where the \replaced{the lapse rate regime is mixed: more stable than a moist adiabat but does not exhibit a surface inversion}{free tropospheric lapse rate deviates 13\% to 35\% from a moist adiabat}.
...
To understand the seasonality of energy balance regimes in the high latitudes, we tested the importance of sea ice for a polar ocean (Arctic) and topography for a polar continent (Antarctica) using mechanism denial experiments. In an aquaplanet with thermodynamic sea ice, the high latitudes undergo a regime transition from RAE to RCAE and the aquaplanet captures the seasonality of the Northern high latitudes. \deleted{However, }\replaced{W}{w}ithout sea ice, the aquaplanet \replaced{exhibits a regime transition for shallow (less than 5 m) mixed layer depths, where the RAE regime occurs only during the spring equinox. Thus, sea ice is important for reproducing a realistic seasonality of the high latitude regime transition.}{remained in RCAE yearround confirming the importance of sea ice}. Using the CESM experiments conducted by \cite{hahn2020}, we found that \replaced{Antarctic topography cannot explain the lack of a high latitude regime transition over Antartica.}{the influence of Antarctic topography is confined to winter, and as a result the Southern high latitudes remain in RAE yearround regardless of the presence of Antarctic topography}.

\added{Finally, we showed that climatological energy balance regimes are useful guides to the vertical temperature responses in regions of RCE that remain in RCE and in regions of RAE with little change in $R_1$ with global warming. Under these conditions, regions of modern RCE corresponds to amplified upper tropospheric warming and modern RAE to amplified surface warming. Outside of these regions (i.e., modern RCAE and RAE where $R_1$ changes), the temperature response may be amplified either in the upper troposphere or the surface depending on the nature of the local forcing.}
...
\added{The connection between modern energy balance regimes and the vertical temperature response to global warming demonstrated here using state-of-the-art GCMs complements the findings of \cite{payne2015}. However, a complete understanding of the latitudinal and seasonal structure of the vertical temperature response to global warming requires a more detailed investigation of the change in $R_1$, especially in the Arctic and the Southern midlatitudes. While amplified upper tropospheric warming in the Southern midlatitudes was previously suggested to be associated with moist adiabatic adjustment \citep{frierson2006a, frierson2008}, another plausible hypothesis is the transient response of the ocean. In particular, both the Southern midlatitudes and the Arctic during summertime correspond to areas of suppressed surface warming that is associated with enhanced ocean heat uptake and the ocean circulation's slow response toward equilibrium \citep{armour2016}.} %\cite{frierson2006a} and \cite{frierson2008} suggest that moist convection plays an important role in controlling the midlatitude temperature response over an ocean-dominated surface such as the Southern Hemisphere midlatitudes. However, we propose an alternative hypothesis, that amplified upper tropospheric warming in the Southern midlatitudes is due to suppressed surface warming associated with ocean heat uptake and the long equilibration time scale of the ocean response \citep{}.}

The framework we introduced for quantifying energy balance regimes can be used to explore many interesting areas for future work. First, the framework can be extended to study the zonal structure of energy balance regimes.\added{ In particular, the role of zonal variations in surface heat capacity (land versus ocean) may be explored for the timing and amplitude of the Northern Hemisphere midlatitude regime transition.} Second, the framework can be used to understand the importance of sea ice on RAE and polar amplification in response to CO$_2$ through its control on the albedo and lapse rate feedback \citep{feldl2020}. Finally, the framework can be applied to past and future climates to understand the potential for energy balance regime transitions through time. Previous studies suggest that high latitudes during warm epochs, such as the Eocene, may have been close to RCE \citep{abbot2008a} whereas RAE was more widespread during Snowball Earth \citep{pierrehumbert2005}. These are all exciting areas for future work.
...
\begin{figure}
  ...
  % \caption{(a) \added{Midlatitude (40$^\circ$--60$^\circ$) }$R_1$ seasonality measured by the minimum value of $R_1$ and (b) surface temperature amplitude (half the difference of maximum and minimum temperatures) predicted by the EBM (solid black line) and simulated by AQUA (stars). The orange region denotes the RCE regime.}
  \caption{(a) \added{Midlatitude (40$^\circ$--60$^\circ$) }$R_1$ seasonality measured by the minimum value of $R_1$ \deleted{and (b) surface temperature amplitude (half the difference of maximum and minimum temperatures) }predicted by the EBM (solid black line) and simulated by AQUA (stars).\added{ (b) Seasonality of midlatitude ($40^\circ$--$60^\circ$) $R_1$ for various mixed layer depths in AQUA without sea ice.} The orange region denotes the RCE regime.}
...
\begin{figure}[t]
    \noindent\includegraphics[width=\textwidth]{/project2/tas1/miyawaki/projects/002/figures_post/final/aqua_ice/echam-ice-props.pdf}\\
    \caption{\added{Seasonality of high latitude ($80^\circ$--$90^\circ$) (a) surface albedo, (b) sea ice fraction, and (c) sea ice depth for various mixed layer depths in AQUA with sea ice.}}
...
\begin{figure}[t]
    ...
    \caption{\added{(a) Seasonality of high latitude ($80^\circ$--$90^\circ$) $R_1$ and (b) the boundary layer lapse rate deviation from a moist adiabat for various mixed layer depths in AQUA with ice. (c,d) Similar, but for AQUA without ice.}}
...
% \begin{figure}
%   \centering
%   % \noindent\includegraphics[width=\textwidth]{/project2/tas1/miyawaki/projects/002/figures/gcm/mmm/historical/198001-200512/1.00/dflux/mse_old/lo/0_r1z_mon_lat.png}
%   \noindent\includegraphics[width=0.9\textwidth]{/project2/tas1/miyawaki/projects/002/figures_post/final/r1_rcp85/r1_rcp85.pdf}\\
...
\begin{figure}
  \centering
  % \noindent\includegraphics[width=0.9\textwidth]{/project2/tas1/miyawaki/projects/002/figures/gcm/mmm/historical/198001-200512/1.00/dflux/mse_old/lo/0_r1z_mon_lat_overlay.png}\\
  \noindent\includegraphics[width=0.9\textwidth]{/project2/tas1/miyawaki/projects/002/figures_post/final/dr1_dt/r1_hatched_cmip5_5p.pdf}\\
