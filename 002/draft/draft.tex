...
\section{Introduction}
    ...
    The energy balance of the atmosphere is achieved through three types of thermodynamic heat transfer: radiation, conduction, and convection \citep{hartmann2016}. As the dominant forms of heat transfer are a strong function of latitude, simple climate models commonly assume simplified states of energy balance, or idealized heat transfer regimes. As simple climate models are an integral part of the climate model hierarchy and form the basis of our understanding of the climate system, it is important to understand when and where idealized heat transfer regimes are satisfied.
    
    In the low latitudes, atmospheric radiative cooling is primarily balanced by latent heating associated with convection \citep{hartmann2002}. Thus, Radiative-Convective equilibrium (RCE) has become a standard idealized configuration for modeling the tropics of varying complexity, ranging from 1-D theories \citep{emanuel1996,nilsson1999,romps2014,singh2015} to 3-D cloud resolving model simulations \citep{wing2018}. RCE in the tropics is commonly associated with a convectively neutral temperature profile \citep{singh2013}, which in the simplest form can be characterized as near-moist adiabatic \citep{held1982,robe1996,tompkins1998}.

    In the high latitudes, radiative fluxes are primarily balanced by advective heat transport associated with the large-scale circulation \citep{nakamura1988}. Accordingly, \cite{cronin2016} recently proposed Radiative-Advective Equilibrium (RAE) as an idealized configuration for modeling the vertical temperature profile of the polar regions. Temperature profiles of RAE typically feature a near-surface temperature inversion because the surface can cool efficiency through the atmospheric window and the vertical profile of advective heat transport peaks slightly above the surface \citep{hahn2020}.
    
    Finally in the midlatitudes, all three types of heat transfer are important. We introduce the term Radiative-Convective-Advective Equilibrium (RCAE) to describe this heat transfer regime. Temperature profiles of RCAE are more stable than a moist adiabat \citep{schneider2008} due to the sensible and latent heating associated with baroclinic eddies.
    
    Categorizing latitudinal regions based on heat transfer regimes makes it straightforward to understand the temperature response to perturbations. For example, the warming response to increased CO$_2$ maximizes aloft in the tropics, which is characteristic of the RCE moist adiabatic adjustment \citep{held1993a,romps2011}. In contrast, the warming response maximizes at the surface in the polar regions consistent with the near-surface stable lapse rate of RAE \citep{payne2015,cronin2016}. Heat transfer regimes provide a quantitative definition for the boundaries of tropical and polar regions, which are known to change with forcing. For example, the RCE tropics expand in response to increased CO$_2$ \citep{seidel2007} and the RAE polar regions expand during Snowball Earth \citep{pierrehumbert2005}.
    
    Although the importance of understanding the spatio-temporal structure of idealized heat transfer regimes are clear, to date very few studies have quantified when and where RCE and RAE are observed. In particular, \cite{jakob2019} quantifies the spatial and temporal scales where RCE is approximately satisfied in the annual mean by using the dry static energy (DSE) budget. They define approximate RCE as where the DSE flux convergence is less than 50 Wm$^{-2}$. However, the DSE framework is problematic for identifying heat transfer regimes outside of the tropics because it neglects latent heat transport as a form of poleward energy transport. Furthermore, a fixed dimensional threshold does not work as a general criteria, such as when considering heat transfer regimes in the context of climate change.
    
    Here, we develop a new framework for defining heat transfer regimes for quantifying when and where RCE and RAE regimes occur in the annual mean and seasonally for the modern Earth as a first step toward understanding changes in different climates. Heat transfer regimes are defined using a nondimensional number that arises upon nondimensionalizing the moist static energy (MSE) budget by radiative cooling. Our approach is complementary to using the nondimensional Rossby number to define angular momentum conserving and eddy driven regimes of the momentum budget \citep{schneider2006}.

\section{Methods}

    \subsection{Defining heat transfer regimes using the MSE budget}

    The zonal mean, vertically integrated MSE equation is: 
    \begin{equation} \label{eq:mse}
        \frac{\partial \langle h \rangle}{\partial t} + \nabla\cdot \langle F_{m} \rangle = R_{a} + \mathrm{LH+SH} \, ,
    \end{equation}
    where $h=c_p T + gz + Lq$ is MSE, $F_m$ is the MSE flux, $R_a$ is radiative cooling, $\mathrm{LH}$ is surface latent flux, $\mathrm{SH}$ is surface sensible heat flux, and $\langle \cdot \rangle$ is the mass-weighted vertical integral \citep{neelin1987}. We nondimensionalize Equation (\ref{eq:mse}) by dividing by radiative cooling, \(R_{a}\), which is negative definite on modern Earth. The nondimensionalized MSE budget is
    \begin{equation}
    ...
    \end{equation}
    In the annual mean, $R_1\approx\frac{\nabla\cdot F_m}{R_a}$ and thus quantifies the relative importance of horizontal fluxes through an atmospheric column and $R_2$ quantifies the relative importance of vertical fluxes through the atmospheric column. The same interpretation of $R_1$ also applies through the seasonal cycle in the low latitudes where $\partial_t h$ is one order of magnitude smaller than $\nabla\cdot F_m$. However, in the mid and high latitudes, $|\partial_t h|\sim 50$ Wm$^{-2}$ around equinox and thus $R_1$ quantifies the relative importance of both horizontal fluxes and the MSE tendency. Our choice to include $\partial_t h$ in the definition of $R_1$ is motivated by the fact that it is indistinguishable from the advective term in the Lagrangian perspective of an atmospheric column and that the spatio-temporal structure of $R_1$ defined this way is consistent with the spatio-temporal structure of lapse rate regimes.
    
    Strict radiative convective equilibrium requires that all fluxes in the column be oriented in the vertical direction (i.e., \(R_{1}=0\)). This is only satisfied in the global mean and where the MSE flux divergence changes sign in the midlatitudes, so we define approximate radiative convective equilibrium as regions where the MSE flux convergence is negligibly small or the MSE flux is divergent (\(R_{1}\le \epsilon\)). We choose to allow all magnitudes of positive MSE flux divergence because the MSE flux divergence in the deep tropics is comprised of upper level divergence and lower level convergence, both of which destabilizes the column to convection. That is, although states of large negative $R_1$ do not amount to strict RCE from an energy balance perspective, the dynamics and its control on the lapse rate are expected to behave similarly to RCE \citep{warren2020}.
    ...
    We use the ECHAM6 atmospheric general circulation model configured as a slab ocean aquaplanet to determine the dominant physical mechanisms that lead to regime transitions in the annual cycle. In particular, we hypothesize that 1) the mixed layer depth controls the seasonal amplitude and phase of $R_1$ and thus the existence of summertime RCE in the midlatitudes and 2) sea ice is required for wintertime RAE in the high latitudes. ECHAM6 is the atmospheric component of the MPI-ESM-LR GCM \citep{stevens2013}. We vary the mixed layer depth from 2--50 m in the standard configuration of the ECHAM6 aquaplanet without sea ice to test the sensitivity of midlatitude RCE to the surface heat capacity. We run additional experiments with ECHAM6 where thermodynamic sea ice is enabled as in \citep{shaw2020} to test whether sea ice is a necessary condition for wintertime RAE.
