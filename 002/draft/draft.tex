...
\section{Introduction}
    ...
    The Earth's climate is maintained by three dominant types of heat transfer that can be clearly identified in the vertically-integrated moist static energy (MSE) budget:
    ...
    \end{equation}
    where $h=c_p T + gz + Lq$ is MSE, $F_m=\mathbf{v}h$ is the MSE flux and $\mathbf{v}$ is the horizontal velocity vector, $R_a$ is radiative cooling, $\mathrm{LH}$ is surface latent flux, $\mathrm{SH}$ is surface sensible heat flux, and $\langle \cdot \rangle$ is the mass-weighted vertical integral \citep{neelin1987}. In equation~(\ref{eq:mse}), conduction relates to latent heating and thus convection via latent heat flux (evaporation), which is connected to precipitation via the moisture budget. Advection represents horizontal MSE transfer by the atmospheric circulation, including the Hadley circulation and storm tracks.
    
    In the low latitudes, atmospheric radiative cooling is primarily balanced by surface turbulent heat fluxes, i.e. $R_a + \mathrm{LH + SH}$ \citep{riehl1958} and the temperature profile is close to moist adiabatic \citep{stone1979,xu1989}. Radiative-Convective equilibrium (RCE) has become a standard idealized configuration for tropical theories \citep[e.g.,][]{emanuel1996,nilsson1999,romps2014,singh2015} and simulations \citep[][and the references therein]{wing2018}. Temperature profiles in RCE, as in the tropics, are near moist adiabatic \citep[e.g.,][]{held1982,robe1996,tompkins1998}.

    In the high latitudes, atmospheric radiative cooling is primarily balanced by large-scale advection, i.e. $\nabla\cdot \langle F_m \rangle \approx R_a$ \citep{nakamura1988}. Accordingly, \cite{cronin2016} proposed Radiative-Advective Equilibrium (RAE) as an idealized configuration for modeling the vertical temperature profile of the polar regions. Temperature profiles in RAE typically feature a near-surface temperature inversion because the surface can cool efficiency through the atmospheric window and the vertical profile of advective heat transport peaks slightly above the surface \citep{oort1974}.
    
    Finally in the midlatitudes, all three types of heat transfer are important; thus, we introduce the term Radiative-Convective-Advective Equilibrium (RCAE). RCAE is assumed in flux-diffusive Energy Balance Models (EBM), which have been used to study the latitudinal structure of surface temperature across a range of climates \citep{north1975}. The vertical temperature profile of RCAE are more stable than a moist adiabat \citep{stone1979,korty2007} due to the sensible and latent heating associated with baroclinic eddies.
    
    Clearly, latitudinal heat transfer regimes are linked qualitatively to the vertical temperature structure of the atmosphere. This link can then be exploited to understand the temperature response to perturbations in different regions. For example, the warming response to increased CO$_2$ maximizes aloft in the tropics, which is characteristic of RCE and moist adiabatic adjustment \citep{held1993,romps2011,miyawaki2020}. In contrast, the warming response maximizes at the surface in the polar regions consistent with the near-surface stable lapse rate of RAE \citep{manabe1975,held1993,graversen2008}. The link between heat transfer regimes and temperature strcuture can also be exploited to define the boundaries of the tropical and polar regions, which are known to change with forcing. For example, the tropics expand in response to increased CO$_2$ \citep{seidel2007} and the polar regions expand during Snowball Earth \citep{pierrehumbert2005}.
    
    To date, very few studies have sough to quantify the links between heat transfer regimes and temperature structure. More specifically, we lack a complete understanding of where and when RCE and RAE hold quantitatively on the modern Earth. \cite{jakob2019} investigated the spatial and temporal scales where RCE holds in the deep tropics in the annual mean by defining RCE as where the dry static energy (DSE) advective flux divergence is less than 50 Wm$^{-2}$. The DSE framework is problematic for identifying heat transfer regimes outside of the tropics because it neglects latent heat as a form of advective heat flux. Furthermore, a fixed dimensional threshold is problematic as a general criteria, since it may not be robust to climate change.
    
    Here, we develop a new framework for defining heat transfer regimes for quantifying when and where RCE and RAE regimes occur in the annual mean and seasonally for the modern Earth. This is the first step toward understanding regime changes in different climates. Heat transfer regimes are defined using a nondimensionalization of the MSE budget. 

\section{Methods}

    \subsection{Defining heat transfer regimes using the nondimensionalized MSE budget}

     We nondimensionalize the MSE budget (\ref{eq:mse}) by dividing by radiative cooling, \(R_{a}\), a negative definite quantity on modern Earth, to obtain
    \begin{equation}
    ...
    \end{equation}
    where $R_1$ and $R_2$ are nondimensional numbers. 
    
    In the strictest sense, RCE requires that radiative cooling balances convective heating in statistical steady state (i.e., \(R_{1}=0\)). Here, we link convective heat fluxes with the surface turbulent fluxes because the latter destabilizes the column to moist convection by supplying warm, moist air to the lower troposphere. As this is only satisfied in the global mean, we define RCE more broadly as \(R_{1}\le \epsilon\) so as to include regions with upper-level divergence that destabilize the column.
    
    Strict RAE as defined in \cite{cronin2016} requires that radiative cooling is approximately balanced by atmospheric heat transport convergence, while surface turbulent fluxes are assumed to be negligible (i.e., \(R_{2}=0\) or equivalently \(R_{1}=1\)). To be consistent with the approximate definition of RCE, we define approximate RAE as regions where positive surface turbulent fluxes are small or the surface turbulent fluxes are negative (\(R_{2} \ge -\epsilon \) or equivalently \(R_{1} \ge 1-\epsilon\)). Thus, as for RCE, we broaden the definition of RAE.
    
   In order to choose the value for $\epsilon$, we examine the annual mean zonal mean temperature profiles as a function of $R_1$ using ERA5 and CMIP5 data (Fig.~\ref{fig:temp-binned-r1}). The temperature structure is a clear function of $R_1$. The near surface inversion emerges around $R_1=0.9$ and thus we choose $R_1=\epsilon=0.9$ to define the cutoff for the RAE regime. Consistently, we use $R_1=1-\epsilon=0.1$ to define the RCE regime. The temperature profile for $R_1=0.1$ is close to moist adiabatic.
    ...
    \subsection{Reanalysis data and model hierarchy}
    ...
    ERA5 is the latest generation reanalysis product provided by the European Centre for Medium-Range Weather Forecasts \citep{hersbach2020}. In order to calculate $R_1$ from ERA5, we use monthly $R_a$, $\mathrm{LH}$, and $\mathrm{SH}$ and infer $\partial_t \langle h\rangle +\nabla\cdot\langle F_m\rangle$ as the residual. While the surface turbulent fluxes are known to be poorly constrained in reanalyses \citep{dee2021} and thus are usually inferred as the residual \citep[e.g.,][]{trenberth2001,fasullo2008,donohoe2013}, in this study we infer $\partial_t \langle h \rangle + \nabla\cdot\langle F_m \rangle$ as the sum $R_a + \mathrm{LH} + \mathrm{SH}$ to compute $R_1$. We choose to use the output of surface turbulent fluxes here because we are interested in verifying the diagnosed heat transfer regimes with the expected vertical temperature profiles of each regime and the near surface temperature and turbulent fluxes are tightly coupled through the bulk aerodynamic formula parameterization.
    ...
    \subsubsection{ECHAM6 slab ocean aquaplanet experiments}
    ...
    \subsubsection{Seasonal energy balance model}
